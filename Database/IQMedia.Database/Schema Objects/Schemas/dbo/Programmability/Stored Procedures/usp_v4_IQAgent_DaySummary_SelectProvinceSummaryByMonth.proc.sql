CREATE PROCEDURE [dbo].[usp_v4_IQAgent_DaySummary_SelectProvinceSummaryByMonth]  
...
BEGIN  
 ...
 IF(@SearchRequestIDXml IS NOT NULL)  
  BEGIN  
   ...
   GROUP BY   
    IQ_DMAProvinceLookup.Province,CONVERT(date,IQAGENT_TVResults.GMTDatetime),IQAgent_SearchRequest.ClientGUID  
   end  
   else if(@Medium = 'NM')  
   begin  
    SELECT   
     ...
     GROUP BY   
      IQAgent_NMResults.State,CONVERT(date,IQAgent_NMResults.harvest_time),IQAgent_SearchRequest.ClientGUID   
   end    
 END  
 ELSE  
 BEGIN  
