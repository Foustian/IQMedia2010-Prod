CREATE PROCEDURE [dbo].[usp_v5_Dashboard_Archive_SMResults_Select]
(
	@MediaIDXml xml,
	@Source varchar(20),
	@Medium varchar(15),
...
BEGIN
	...
	SELECT top 10 
		actor_preferredusername as actor_preferredname,
		count(ArchiveTweets.ArchiveTweets_Key) as NoOfDocs,
		ISNULL(sum(CONVERT(BIGINT,Number_Hits)),0) as Mentions,
		...
		ISNULL(sum(ArchiveTweets.PositiveSentiment),0) as PositiveSentiment,
		ISNULL(sum(ArchiveTweets.NegativeSentiment),0) as NegativeSentiment
	FROM 
		ArchiveTweets WITH (NOLOCK)
			INNER JOIN IQArchive_Media WITH (NOLOCK)
	...
	ORDER BY ISNULL(sum(CONVERT(BIGINT,Number_Hits)),0) desc,count(ArchiveTweets.ArchiveTweets_Key) desc
END