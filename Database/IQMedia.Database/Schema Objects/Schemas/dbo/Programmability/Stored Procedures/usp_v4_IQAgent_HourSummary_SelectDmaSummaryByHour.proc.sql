CREATE PROCEDURE [dbo].[usp_v4_IQAgent_HourSummary_SelectDmaSummaryByHour]
...
BEGIN  
 ...
 BEGIN  
  ...
  begin  
   ...
   GROUP BY   
     IQ_Station.DMA_Name,DateAdd (hour,DATEPART(hour,IQAGENT_TVResults.GMTDatetime), convert (varchar(10),IQAGENT_TVResults.GMTDatetime,101) ),IQAgent_SearchRequest.ClientGUID  
  end  
  else if(@Medium = 'NM')  
  begin  
    SELECT   
     ...
     GROUP BY   
      IQ_Dma_Num,IQ_DMA_Name,DateAdd (hour,DATEPART(hour,IQAgent_NMResults.harvest_time), convert (varchar(10),IQAgent_NMResults.harvest_time,101) ),IQAgent_SearchRequest.ClientGUID  
  end  
END  
 ELSE  
 BEGIN  
