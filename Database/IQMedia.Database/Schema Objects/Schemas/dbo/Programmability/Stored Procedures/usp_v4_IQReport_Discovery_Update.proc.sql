CREATE PROCEDURE [dbo].[usp_v4_IQReport_Discovery_Update]
	@MediaID varchar(max),
	@ReportID bigint,
...
BEGIN
	...
	IF (SELECT [Status] FROM IQReport_Discovery WHERE ID = @ReportID) = 'QUEUED'
	  BEGIN
		UPDATE IQJob_Master
	...
	ELSE
	  BEGIN				
		...
		(
			...
			@UpdateJobTypeID,
			@CustomerGuid,
			(SELECT Title FROM IQReport_Discovery WHERE ID = @ReportID),
			GETDATE(),
			NULL,
			NULL,
			'QUEUED',
