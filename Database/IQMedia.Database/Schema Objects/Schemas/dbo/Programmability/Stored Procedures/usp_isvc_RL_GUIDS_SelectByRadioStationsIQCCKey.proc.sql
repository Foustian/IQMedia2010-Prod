CREATE PROCEDURE [dbo].[usp_isvc_RL_GUIDS_SelectByRadioStationsIQCCKey]
	@IQCCKey		varchar(Max),
	@PageNumber		int,
	@PageSize		int,
...
BEGIN	
	...
	set @Query='WITH IQMediaRL_GUIDS  AS ( Select  ROW_NUMBER() OVER (Order by '
			...
			IF @SortField IS NOT NULL AND @SortField <> ''
				BEGIN
						SET @SortField=REPLACE(@SortField,'DateTime',' CONVERT(datetime,CONVERT(varchar(Max),RL_GUIDS.RL_Station_Date,101) + '' ''+ REPLACE(convert(varchar(Max),convert(decimal(4,2),(convert(decimal(6,2),RL_GUIDS.RL_Station_Time)/convert(decimal(5,2),100)))),''.'','':'')+'':00'') ')
	 ...
	 set @Query=@Query +'Convert(uniqueidentifier,RL_GUIDS.RL_GUID) as RL_GUID,
			...
			dma_name,
			CONVERT(varchar(Max),CONVERT(varchar(Max),RL_GUIDS.RL_Station_Date,101) + '' ''+ REPLACE(convert(varchar(Max),convert(decimal(4,2),(convert(decimal(6,2),RL_GUIDS.RL_Station_Time)/convert(decimal(5,2),100)))),''.'','':'')+'':00'') as DateTime
	From
			RL_GUIDS
