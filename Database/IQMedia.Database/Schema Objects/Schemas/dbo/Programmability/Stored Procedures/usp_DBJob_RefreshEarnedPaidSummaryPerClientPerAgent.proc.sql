...
/****** Object:  StoredProcedure [dbo].[usp_DBJob_RefreshEarnedPaidSummaryPerClientPerAgent]    Script Date: 10/20/2016 3:11:54 PM ******/
SET ANSI_NULLS ON
GO
...
DECLARE @InsertStatus CHAR(50)
EXEC usp_DBJob_RefreshEarnedPaidSummaryPerClientPerAgent @ClientGUID,@AgentID,NULL,@InsertStatus=@InsertStatus output
SELECT @InsertStatus
*/
CREATE PROCEDURE [dbo].[usp_DBJob_RefreshEarnedPaidSummaryPerClientPerAgent]  (      
@ClientGUID UNIQUEIDENTIFIER,@AgentID BIGINT, @TVID BIGINT , @InsertStatus CHAR(50) OUTPUT)
AS       
BEGIN         
 SET NOCOUNT ON;        
 SET XACT_ABORT ON;
 ...
 BEGIN TRY        
   ...
   CREATE TABLE #IQAgent_EarnedPaidSummary
    ...
    IF NOT EXISTS (SELECT * FROM IQAgent_SearchRequest WITH (NOLOCK) WHERE ClientGUID=@ClientGUID AND ID = @AgentID)
      BEGIN
	     PRINT 'Client ' + CONVERT(VARCHAR(50),@ClientGUID) + ' ' + 'OR ' + CONVERT(VARCHAR(50),@AgentID) + ' DOES NOT EXIST IN THE IQAgent_SearchRequest'
		 RETURN 0
	  END
   ...
   INSERT INTO #IQAgent_TVResults
	...
	EXEC @Status = usp_DBJob_RefreshEarnedPaidSummaryPerClientPerAgent_UpdateTables  @ClientGUID, @AgentID WITH RECOMPILE
	IF @Status = 0
  ...
  BEGIN CATCH        
   ...
   DECLARE @IQMediaGroupExceptionKey BIGINT,
		...
		SELECT 
				...
				@CreatedBy='usp_DBJob_RefreshEarnedPaidSummaryPerClientPerAgent',
				@ModifiedBy='usp_DBJob_RefreshEarnedPaidSummaryPerClientPerAgent',
				@CreatedDate=GETDATE(),
