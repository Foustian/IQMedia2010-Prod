CREATE PROCEDURE [dbo].[usp_v4_IQ_Report_SelectIQAgentReportByReportGUID]
...
BEGIN
	...
	IF (@ReportType='HOURLY' AND @ReportRule.exist('Report/IQAgent/SearchRequestIDList/SearchRequestID/Results')=1)
		BEGIN			
			EXEC usp_v4_IQ_Report_SelectIQAgentReportByReportGUID_Hourly @ReportGUID,@MaxDisplayRecord,@SearchRequestID,@MediaType,@IsSourceEmail,@UseRollup OUTPUT
	...
	ELSE																			 		
		...
		BEGIN
			...
			IF EXISTS(SELECT 1 FROM #TblMediaTypes AS ReportConfig WHERE MediaType = 'News')
				BEGIN
					if(@UseRollup!=1)
					begin
						;With TempNM AS(
			...
			IF EXISTS(SELECT 1 FROM #TblMediaTypes AS ReportConfig WHERE MediaType = 'SocialMedia')
				BEGIN
					;With TempSocialMedia AS(
						...
						AND		IQAgent_MediaResults.Category IN ('SocialMedia','FB','IG')
						AND		IQAgent_MediaResults.MediaDate BETWEEN @FromDate AND @ToDate
			...
			IF EXISTS(SELECT 1 FROM #TblMediaTypes AS ReportConfig WHERE MediaType = 'Blog')
				BEGIN
					;With TempBlog AS(
						...
						AND		IQAgent_MediaResults.Category = 'Blog'
						AND		IQAgent_MediaResults.MediaDate BETWEEN @FromDate AND @ToDate
			...
			IF EXISTS(SELECT 1 FROM #TblMediaTypes AS ReportConfig WHERE MediaType = 'Forum')
				BEGIN
					;With TempForum AS(
						...
						AND		IQAgent_MediaResults.Category = 'Forum'
						AND		IQAgent_MediaResults.MediaDate BETWEEN @FromDate AND @ToDate
			...
			IF EXISTS(SELECT 1 FROM #TblMediaTypes AS ReportConfig WHERE MediaType = 'Twitter')
				BEGIN
					;With TempTW AS(
						SELECT 
								ROW_NUMBER() OVER (PARTITION BY IQAgent_SearchRequest.ID ORDER BY MediaDate DESC) As RowNum,
			...
			IF EXISTS(SELECT 1 FROM #TblMediaTypes AS ReportConfig WHERE MediaType = 'TM')
				BEGIN
					;With TempTM AS(
						SELECT 
									ROW_NUMBER() OVER (PARTITION BY IQAgent_SearchRequest.ID ORDER BY MediaDate DESC) As RowNum,
				...
				BEGIN
					;With TempPQ AS(
						SELECT 
							...
							AND		IQAgent_MediaResults.MediaType = 'PQ'
