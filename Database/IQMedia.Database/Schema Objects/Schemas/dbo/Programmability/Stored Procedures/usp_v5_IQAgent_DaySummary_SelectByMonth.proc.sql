CREATE PROCEDURE [dbo].[usp_v5_IQAgent_DaySummary_SelectByMonth]
(
...
BEGIN
	...
	SET @SPName ='usp_v5_IQAgent_DaySummary_SelectByMonth'   
	...
	BEGIN
		...
		Group By	
				Datepart(Year,DayDate),Datepart(Month,DayDate),IQAgent_DaySummary.ClientGuid,IQAgent_SearchRequest.ID,IQAgent_SearchRequest.Query_Name,
				MediaType,
				IQAgent_DaySummary.SubMediaType

		SET @QueryDetail ='get all monthly summary'
	...
	BEGIN
		...
		Group By	
				Datepart(Year,DayDate),Datepart(Month,DayDate),IQAgent_DaySummary.ClientGuid,
				MediaType,
				IQAgent_DaySummary.SubMediaType
	...
	SET @DayDifference  = DATEDIFF(MONTH,@FromDate,@ToDate) + 1

	DECLARE @FromDatePrev date
	DECLARE @ToDatePrev date

	SET @FromDatePrev = DATEADD(MONTH,-@DayDifference,@FromDate)
	SET @ToDatePrev = DATEADD(MONTH,-@DayDifference,@ToDate)
