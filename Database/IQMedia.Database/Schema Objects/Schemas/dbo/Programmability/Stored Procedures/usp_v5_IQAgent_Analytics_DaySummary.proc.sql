USE [IQMediaGroup]
GO
/****** Object:  StoredProcedure [dbo].[usp_v5_IQAgent_Analytics_DaySummary]    Script Date: 12/15/2016 10:45:39 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[usp_v5_IQAgent_Analytics_DaySummary]
(
...
BEGIN
	...
	SET @SPName ='usp_v5_IQAgent_Analytics_DaySummary'  
	...
	IF @ClientGUID IN ('7722A116-C3BC-40AE-8070-8C59EE9E3D2A', '1D0A3DD5-47B7-4316-B3AD-10C018CA503D', '36E623A5-FDD2-4FF6-802F-317D12B6415A')
	  BEGIN
			...
			SELECT	
				...
				CtPositiveSentiment, 
				CtNegativeSentiment, 
				CtSeen_Earned + CtSeen_Inprogram, 
				CtSeen_Paid, 
				CtHeard_Earned + CtHeard_Inprogram, 
				CtHeard_Paid, 
	...
	GROUP BY
		DayDate, tempData.ClientGuid, IQAgent_SearchRequest.ID, IQAgent_SearchRequest.Query_Name, SubMediaType, Market
	...
	BEGIN
		...
		SELECT
			...
			DMA_Name,
			IQAgent_SearchRequest.ID,
			ISNULL(sum(CONVERT(BIGINT,Number_Hits)),0) + ISNULL(SUM(CONVERT(BIGINT, LRR.NumOfHits)),0)
		FROM 
