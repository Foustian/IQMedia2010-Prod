CREATE PROCEDURE [dbo].[usp_ArchiveSM_SelectByCategory]
...
BEGIN
    ...
    SET @Query = @Query + 	') as RowNumber,
		...
		END
			...
			ANd ARchiveSM.CategoryGuid = '''+ CONVERT(varchar(40),@CategoryGUID) +'''
			AND CONVERT(date,ARchiveSM.CreatedDate) = CONVERT(date,'''+CONVERT(varchar(10),@ArticleDate) +''')
