CREATE PROCEDURE [dbo].[usp_v4_IQClipTimeSync_Data_SelectByClipGuidAndCustomerGuid]
...
BEGIN
	...
	BEGIN
		...
		SELECT 
			IQClipTimeSync_Data.Data,
			IQTimeSync_Type.Name,
			IQTimeSync_Type.GraphStructure
		FROM 
			IQClipTimeSync_Data
				inner join IQTimeSync_Type 
					on IQClipTimeSync_Data._TypeID = IQTimeSync_Type.ID
					and IQClipTimeSync_Data.IsActive = 1
					and IQTimeSync_Type.IsActive = 1
		WHERE
			IQClipTimeSync_Data._ClipGuid = @ClipGuid
			and _TypeID in (
				SELECT 
						SplitTbl.Items
					from 
