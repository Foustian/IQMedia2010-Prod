...
/****** Object:  StoredProcedure [dbo].[usp_v4_IQAgent_MediaResults_Dirtytbl_Process_Delete]    Script Date: 8/12/2015 10:51:42 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_v4_IQAgent_MediaResults_Dirtytbl_Process_Delete]        
...
BEGIN         
 ...
 BEGIN TRY        
    ...
    Select  @StartID = min(ID) from dbo.IQAgent_MediaResults_DirtyTable With (NoLock) where IsActive=0
...
*/
 IF (select count(1) from #MediaResults_Delete) > 0
    Begin
       ...
       Create index idx1_MediaResults_Delete on #MediaResults_Delete(MediaType,MediaID)
        Create index idx2_MediaResults_Delete on #MediaResults_Delete(DirtyTableID)
