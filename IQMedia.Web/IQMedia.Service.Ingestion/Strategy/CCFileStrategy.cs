...
{
    public class CCFileStrategy : StrategyBase
    {
        public CCFileStrategy(string fileExt) : base(fileExt)
        {
        }
        ...
        public override void Process(string sourceFile)
        {
            ...
            if (rfList.Count > 0)
            {
                Logger.Warning("Duplicate CC record(s) exist in database. Old recordfile(s) will be set to status 'DUPLICATE'.");
            ...
            var newRecord = new Recordfile
            {
                ...
                RootPath = rPath,
                RootPathID = rPath.ID,
                Status = ConfigSettings.Settings.RecordingStatus_CC
            };
