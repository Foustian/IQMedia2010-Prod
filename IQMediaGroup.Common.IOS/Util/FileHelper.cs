...
namespace IQMediaGroup.Common.IOS.Util
{
    public static class FileHelper
    {
        ...
        public static bool CopyFile(string sourceFile, string destinationFile, int bufferSize = (128*1024))
        {
            ...
            {
                Log4NetLogger.Warning(String.Format("Source file '{0}' or Destination file '{1}' was not valid.", sourceFile, destinationFile));
                return false;
            }
            ...
            try
            {
                ...
                {
                    Log4NetLogger.Warning("Error creating directory: " + destinationPath);
                ...
                //Log Progess
                Log4NetLogger.Debug(String.Format("Copying file from '{0}' destination '{1}'", sourceFile, destinationFile));
                ...
                if (File.Exists(destinationFile))
                {
                    ...
                    Log4NetLogger.Info(String.Format("File '{0}' successfully copied to destination '{1}'", sourceFile, destinationFile));
                    success = true;
                }
                else
                {
            ...
            {
                Log4NetLogger.Warning(String.Format("The source file '{0}' does not exist on the file system.", sourceFile), ex);
            }
            catch (IOException ex)
            {
                Log4NetLogger.Warning(String.Format("The source file '{0}' is in use by another thread and will be skipped.", sourceFile), ex);
            }
            catch (Exception ex)
            {
                Log4NetLogger.Error(String.Format("Error copying '{0}' to '{1}'.", sourceFile, destinationFile), ex);
            }
            finally
            {
        ...
        public static bool DeleteFile(string filePath)
        {
            try
            {
                if (File.Exists(filePath))
                {
                    ...
                    Log4NetLogger.Info(String.Format("File '{0}' successfully deleted.", filePath));
                    return true;
                }

                Log4NetLogger.Info(String.Format("File '{0}' does not exist and doesn't need to be deleted.", filePath));
                return true;
            }
            catch (Exception ex)
            {
                Log4NetLogger.Error("Error deleting file: " + filePath, ex);
                return false;
            }
        }
        ...
        public static bool CreateTextFile(string filename, string contents)
        {
            ...
            {
                Log4NetLogger.Error("An error occurred while attempting to create file: " + filename, ex);
                return false;
            }
        }  
