        ...
        private void ProcessDBRecords(FeedsExportTask p_FeedsExportTask, CancellationToken p_CToken)
        {
            try
            {
                ...
                if (p_FeedsExportTask.GetTVUrl)
                {
                    ...
                    // Generating TV urls takes an extremely long time, whereas generating the CSV file is very fast. 
                    ...
                    foreach (XElement element in xDoc.Descendants("TVUrl"))
                    {
                        ...
                        if (element.Descendants("Processed").First().Value == "0")
                        {
                            ...
                            while (isRetry && numTries < 5)
                            {
                                ...
                                response = feedsLogic.DoHttpGetRequest(
                                    String.Format(ConfigurationManager.AppSettings["UrlGetIQAgentIframeUrl"],
                                        DateTime.Now.AddDays(clientSettings.RawMediaExpiration.Value).ToShortDateString(), mediaID, String.Empty));
                            ...
                            if (numTries > 1)
                            {
                                Logger.Info(String.Format("{0}TVResults ID: {1} - {2} attempts to get raw media url.", logPrefix, mediaID, numTries));
                            }
