...
using System.Xml;
using IQMedia.Service.ArchiveMetaDataUpdate.Config;
...
{
    public partial class ArchiveMetaDataUpdate : ServiceBase
    {
        private Thread _workerThread;
        private ServiceHost _host;
        private Queue<ArchiveMetaDataUpdateTask> _queueOfMetaDataUpdateTask;
        ...
        protected static void InitializeService()
        {
            ...
            ConfigurationManager.RefreshSection("ArchiveMetaDataUpdateSettings");
            ...
            if (ConfigSettings.Settings == null)
                throw new XmlException("App.config is missing <ArchiveMetaDataUpdateSettings> node.");
        ...
        public void Run()
        {
            try
            {
                LogMessage("INFO", "ArchiveMetaDataUpdate Service started at: " + DateTime.Now);
                //var baseAddress = new Uri("http://localhost:" + ConfigSettings.Settings.WCFServicePort + "/ArchiveMetaDataUpdateWebService");

                //_host = new ServiceHost(typeof(Service.ArchiveMetaDataUpdateWeb), baseAddress);
            ...
            {
                CommonFunctions.WriteException(_connStr, "ArchiveMetaDataUpdate", ex);
        ...
        private void StartTasks()
        {
            while (true)
            {
                ...
                while (_queueOfMetaDataUpdateTask.Count > 0)
                {
                    ...
                    {
                        ArchiveMetaDataUpdateTask metadataUpdateTask = (ArchiveMetaDataUpdateTask)tsk.AsyncState;

                        LogMessage("DEBUG", "Tsk Status - " + tsk.Status, metadataUpdateTask.ID);
                        LogMessage("DEBUG", "ArchiveMetaDataUpdateTsk Status - " + metadataUpdateTask.Status, metadataUpdateTask.ID);
                    }
                ...
                LogMessage("INFO", "Archive MetaData Update Service Enqueuer sleeping until " + DateTime.Now.AddSeconds(sleepTime).ToString("hh:mm:ss tt"));
        ...
        private void GetRecordsFromDB()
        {
            try
            {
                _queueOfMetaDataUpdateTask = new Queue<ArchiveMetaDataUpdateTask>();
                ...
                using (var conn = new SqlConnection(_connStr))
                {
                    ...
                    var cmdStr = "usp_metadataupd_IQService_ArchiveMetaDataUpdate_SelectQueued";
                    ...
                    using (var cmd = conn.GetCommand(cmdStr, CommandType.StoredProcedure))
                    {
                        ...
                        while (reader.Read())
                        {
                            ArchiveMetaDataUpdateTask task = new ArchiveMetaDataUpdateTask
                            {
                                ID = !reader.IsDBNull(0) ? reader.GetInt64(0) : 0,
                                ArchiveTracking = !reader.IsDBNull(1) ? reader.GetString(1) : String.Empty
                            };

                            if (task.ID > 0 && !String.IsNullOrEmpty(task.ArchiveTracking))
                            {
                                _queueOfMetaDataUpdateTask.Enqueue(task);
                            }
                            else
                            {
                                LogMessage("ERROR", "This task contains no ArchiveTracking data.", task.ID);
                                if (task.ID > 0)
                                {
                                    UpdateTaskStatus(ArchiveMetaDataUpdateTask.TskStatus.FAILED.ToString(), task.ID);
                                }
                            }
                        }
        ...
        private void ProcessDBRecords(ArchiveMetaDataUpdateTask p_ArchiveMetaDataUpdateTask, CancellationToken p_CToken)
        {
            try
            {
                LogMessage("INFO", "Task is processing.", p_ArchiveMetaDataUpdateTask.ID);

                p_ArchiveMetaDataUpdateTask.Status = ArchiveMetaDataUpdateTask.TskStatus.METADATA_IN_PROCESS;
                UpdateTaskStatus(ArchiveMetaDataUpdateTask.TskStatus.METADATA_IN_PROCESS.ToString(), p_ArchiveMetaDataUpdateTask.ID);
                ...
                foreach (XElement xElem in xDoc.Descendants("SubMedia"))
                {
                    ...
                    if (p_CToken.IsCancellationRequested)
                    {
                        ...
                        p_ArchiveMetaDataUpdateTask.Status = ArchiveMetaDataUpdateTask.TskStatus.TIMEOUT_METADATA;
                    ...
                    {
                        string newsContent = feedReportLogic.GetNewsContent(xElem.Descendants("ArticleID").FirstOrDefault().Value);
                    ...
                    {
                        string smContent = feedReportLogic.GetSocialMediaContent(xElem.Descendants("ArticleID").FirstOrDefault().Value);
                    ...
                    else if (xElem.Parent.Name == "PQ" && xElem.Descendants("Content").FirstOrDefault().Value == "1")
                    {
                        string pqContent = feedReportLogic.GetProQuestContent(xElem.Descendants("ArticleID").FirstOrDefault().Value);

                        if (string.IsNullOrWhiteSpace(pqContent))
                        {
                            pqContent = string.Empty;
                            xElem.Descendants("Content").FirstOrDefault().Value = "1";
                            LogMessage("INFO", "content is empty for ProQuest _ArchiveMediaID:" + xElem.Descendants("ID").FirstOrDefault().Value, p_ArchiveMetaDataUpdateTask.ID);
                        }
                        else
                        {
                            xElem.Descendants("Content").FirstOrDefault().Value = "0";
                            LogMessage("INFO", "ProQuest ArticleContent :" + (pqContent.Length > 300 ? pqContent.Substring(0, 300) + "...." : pqContent), p_ArchiveMetaDataUpdateTask.ID);
                            LogMessage("INFO", "update archive metadata content for ProQuest _ArchiveMediaID: " + xElem.Descendants("ID").FirstOrDefault().Value, p_ArchiveMetaDataUpdateTask.ID);
                            UpdateArchiveMediaRecord(xElem.Descendants("ID").FirstOrDefault().Value, "PQ", pqContent);
                        }
                    }
                }
                ...
                if (isCompleted)
                {
                    p_ArchiveMetaDataUpdateTask.Status = ArchiveMetaDataUpdateTask.TskStatus.COMPLETED;
                    UpdateArchiveTracking(p_ArchiveMetaDataUpdateTask.ID, xDoc.ToString(), p_ArchiveMetaDataUpdateTask.Status.ToString());
                }
            }
            catch (Exception ex)
            {
                p_ArchiveMetaDataUpdateTask.Status = ArchiveMetaDataUpdateTask.TskStatus.EXCEPTION_METADATA;
                UpdateTaskStatus(ArchiveMetaDataUpdateTask.TskStatus.EXCEPTION_METADATA.ToString(), p_ArchiveMetaDataUpdateTask.ID, true);

                CommonFunctions.WriteException(_connStr, "ArchiveMetaDataUpdate", ex, p_ArchiveMetaDataUpdateTask.ID);
                LogMessage("ERROR", "An error occurred while processing feeds metadata update task.", p_ArchiveMetaDataUpdateTask.ID, ex);
            }
        }
        ...
        private void UpdateArchiveTracking(long taskID, string archiveTrackingXml, string status)
        {
            using (var conn = new SqlConnection(_connStr))
            {
                ...
                using (var cmd = conn.GetCommand("usp_metadataupd_IQService_ArchiveMetaDataUpdate_UpdateArchiveTracking", CommandType.StoredProcedure))
                {
        ...
        private void UpdateTaskStatus(string p_Status, Int64 p_ID, bool resetMachineName = false)
        {
            using (var conn = new SqlConnection(_connStr))
            {
                ...
                using (var cmd = conn.GetCommand("usp_metadataupd_IQService_ArchiveMetaDataUpdate_UpdateStatus", CommandType.StoredProcedure))
                {
        ...
        public void Quit()
        {
            LogMessage("INFO", "ArchiveMetaDataUpdate Service stopped at: " + DateTime.Now);
        }
    }
}
