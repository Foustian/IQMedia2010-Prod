...
namespace IQMedia.Web.Logic
{
    public class AnalyticsLogic: IQMedia.Web.Logic.Base.ILogic
    {
        ...
        private AnalyticsPESHFilters GetPESHFilters(List<string> PESHTypes, List<string> sourceGroups)
        {
            ...
            filters.isHeardInProgram = false;
            filters.isSeenInProgram = false;
            ...
            if (PESHTypes != null && PESHTypes.Count > 0)
            {
                ...
                bool isInProgram = PESHTypes.Contains("InProgram");
                ...
                // In Program subset of earned
                filters.isHeardInProgram = (isHeard && isInProgram) || (isInProgram && !isHeard && !isSeen && !isRead);
                filters.isSeenInProgram = (isSeen && isInProgram) || (isInProgram && !isHeard && !isSeen && !isRead);
        ...
        private decimal GetSumsFromSummaries(AnalyticsRequest request, List<IQ_MediaTypeModel> lstSubMediaTypes, AnalyticsPESHFilters peshFilters, IEnumerable<AnalyticsSummaryModel> summaries)
        {
            ...
            if (request.Tab != SecondaryTabID.Overview)
            {
                ...
                else
                {
                    ...
                    if (peshFilters.isSeenInProgram)
                    {
                        returnSum += summaries.Sum(s => s.SeenInProgram);
                    }
                    if (peshFilters.isHeardInProgram)
                    {
                        returnSum += summaries.Sum(s => s.HeardInProgram);
                    }
                }
            }
            else
            ...
            try
            {
                ...
                foreach (var summary in summaries)
                {
                    if (PESHR.isFiltering)
                    {
                        ...
                        bool seenInProgram = PESHR.isSeenInProgram && summary.SeenInProgram > 0;
                        bool heardInProgram = PESHR.isHeardInProgram && summary.HeardInProgram > 0;

                        if (seenEarned || seenPaid || heardEarned || heardPaid || readEarned || seenInProgram || heardInProgram)
                        {
        ...
        public List<FacetResponse> AnalyticsSearch(AnalyticsRequest request, Guid clientGUID)
        {
            try
            {
                Log4NetLogger.Debug(string.Format("Logic.AnalyticsSearch: {0}", clientGUID.ToString()));
                string url = string.Format("http://10.100.1.161:8080/solr/{0}/select?", request.DateInterval.Equals("day") ? "AnalyticsDaySummary" : "AnalyticsHourSummary");
                Log4NetLogger.Debug(string.Format("Query Solr: {0}", url));
