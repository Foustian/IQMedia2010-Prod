...
namespace IQMedia.WebApplication.Controllers
{
    ...
    public class AnalyticsController : Controller
    {
                    ...
                    foreach (string column in columnHeader)
                    {
                        ...
                        switch (column)
                        {
                            ...
                            case "in program":
                                if (sessionInfo.Isv5AdsAccess && !hideLRAds)
                                {
                                    tc.InnerText = string.Format("{0:N0}", group.Summaries.Sum(s => s.InProgram)); ;
                                    tc.ID = string.Format("INPROGRAM_{0}", group.ID);
                                }
                                break;
                            case "on air time":
        ...
        public ContentResult SearchSolrCloud(string tab, string interval)
        {
            try
            {
                sessionInfo = ActiveUserMgr.GetActiveUser();
                ...
                {
                    case "OverTime":
                        request.Tab = SecondaryTabID.OverTime;
                        break;
                    case "Demographic":
                        request.Tab = SecondaryTabID.Demographic;
                        break;
                    case "Sources":
                        break;
                }

                List<FacetResponse> response = logic.AnalyticsSearch(request, sessionInfo.ClientGUID);
