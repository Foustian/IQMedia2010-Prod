        ...
        public ContentResult GetReport(DateTime startDate, DateTime endDate, string report, string cohortID = null, string subGroup = null)
        {
            try
            {
                ...
                PESHGroupings.Add(new AnalyticsGrouping() {
                    ID = "InProgram",
                    Name = "InProgram",
                    Summaries = requestedData.SummaryDataList.Where(w => w.InProgram > 0).ToList()
                });
                ...
                if (report == "Brand")
                {
                    agentGroupings.Add(new AnalyticsGrouping() {
                        ID = "Heard",
                        Name = "Heard",
                        Summaries = requestedData.SummaryDataList.Where(w => w.HeardEarned > 0 || w.HeardPaid > 0 || w.HeardInProgram > 0).ToList()
                    });

                    agentGroupings.Add(new AnalyticsGrouping() {
                        ID = "Seen",
                        Name = "Seen",
                        Summaries = requestedData.SummaryDataList.Where(w => w.SeenEarned > 0 || w.SeenPaid > 0 || w.SeenInProgram > 0).ToList()
                    });
