...
function OpenFeed() {
    var point = this;
    var sID = "";
    var allowDPDD = _tab !== "Daypart" || point.y !== 2;    // y === 2 is a blank row
    var tab = _tab;
    ...
    if (!sID.endsWith("_overlay") && allowDPDD)
    {
        ...
        $.when(OpenFeedHelper()).then(
            function (status) {
                ...
                if ($.inArray("InProgram", _ActivePESHTypes) > -1)
                {
                    mediumQS += '&inprogram=true';
                }
...
function UpdatePESHFilters(id, checked) {
    ...
    if (_IsAdsAccess)
    {
        ...
        //INPROGRAM
        oldNum = Number($("#primaryNavInProgram_Results").text().replace(/,/g, ''));
        changeNum = Number($("#INPROGRAM_" + id).text().replace(/,/g, ''));
        newNum = 0;
        if (checked)
        {
            newNum = oldNum + changeNum;
        ...
        $("#primaryNavInProgram_Results").text(addCommas(newNum));
    }
...
function toggleEnabledAndHidden() {
    ...
    if (_IsAdsAccess) {
        ...
        $("#primaryNavInProgram").show();
    }
    else {
        $("#primaryNavPaid").hide();
        $("#primaryNavEarned").hide();
        $("#primaryNavInProgram").hide();
    }
    if (_IsLRAccess) {
        $("#primaryNavSeen").show();
...
var _queryResponse = null;
function QuerySolr(interval) {
    ...
    $.ajax({
        url: "/Analytics/SearchSolrCloud/",
        contentType: "application/json; charset=utf-8",
        type: "POST",
        data: JSON.stringify(postData),
        dataType: "json",
        success: function (result) {
            console.log("Query is" + (result.isSuccess ? "" : " not") + " successful");
            if (result.isSuccess)
            {
                _queryResponse = result.response;
            }
            else
            {
