        ...
        public List<SummaryReportModel> GetSummaryReportData(Guid p_ClientGUID, DateTime p_FromDate, DateTime p_ToDate, Int16 p_SearchType, List<string> p_SearchRequestIDs, out IQAgent_DashBoardPrevSummaryModel iQAgent_DashBoardPrevSummaryModel)
        {
            try
            {
                ...
                if (p_SearchType == 0)
                {
                    ...
                    List<GoogleSummaryModel> googleData = googleLogic.GetGoogleDataByHour(p_ClientGUID, p_FromDate, p_ToDate);
                    // Google data is stored in local time, so convert to GMT
                    foreach (GoogleSummaryModel gsm in googleData)
                    {
                        gsm.DayDate = Utility.CommonFunctions.GetGMTandDSTTime(gsm.DayDate).Value;
                    }
                    List<IQAgent_DaySummaryModel> googleDaySummary = googleData.Select(s => new IQAgent_DaySummaryModel()
                    {
                        DayDate = s.DayDate,
                        NoOfDocs = s.NoOfDocs,
                        MediaType = s.MediaType,
                        Query_Name = s.DataType
                    }).ToList();

                    if (iQAgent_DashBoardModel.ListOfIQAgentSummary != null)
                    {
                        iQAgent_DashBoardModel.ListOfIQAgentSummary.AddRange(googleDaySummary);
                    }
                    else
                    {
                        iQAgent_DashBoardModel.ListOfIQAgentSummary = googleDaySummary;
                    }
                ...
                else if (p_SearchType == 1)
                {
                    ...
                    List<GoogleSummaryModel> googleData = googleLogic.GetGoogleDataByDay(p_ClientGUID, p_FromDate, p_ToDate);
                    List<IQAgent_DaySummaryModel> googleDaySummary = googleData.Select(s => new IQAgent_DaySummaryModel()
                    {
                        DayDate = s.DayDate,
                        NoOfDocs = s.NoOfDocs,
                ...
                else if (p_SearchType == 3)
                {
                    ...
                    List<GoogleSummaryModel> googleData = googleLogic.GetGoogleDataByMonth(p_ClientGUID, p_FromDate, p_ToDate);
                    List<IQAgent_DaySummaryModel> googleDaySummary = googleData.Select(s => new IQAgent_DaySummaryModel()
                    {
                        DayDate = s.DayDate,
                        NoOfDocs = s.NoOfDocs,
