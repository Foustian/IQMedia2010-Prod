    ...
    public class AnalyticsDA : IDataAccess
    {
        public List<AnalyticsCampaign> GetCampaigns(Guid clientGUID)
        {
            try
            {
                ...
                return FillAnalyticsCampaigns(dsSSP);
            }
            catch (Exception exc)
            {
        ...
        public AnalyticsDataModel GetDaySummaryData(Guid clientGUID, string searchRequestXml, string subMediaType, bool loadEverything = true)
        {
            try
            {
                ...
                dataModel = FillIQAgentSummary(dsSSP);
                return dataModel;
            }
            catch (Exception ex)
            {
        ...
        public AnalyticsDataModel GetHourSummaryData(Guid clientGUID, string searchRequestXml, string subMediaType, bool loadEverything = true, string GroupByHeader = "")
        {
            try
            {
                ...
                dataTypes.Add(new DataType("@SearchRequestIDXml", DbType.Xml, searchRequestXml, ParameterDirection.Input));
                dataTypes.Add(new DataType("@LoadEverything", DbType.Boolean, loadEverything, ParameterDirection.Input));
                //dataTypes.Add(new DataType("@GroupByHeader", DbType.String, GroupByHeader, ParameterDirection.Input));
        ...
        public AnalyticsDataModel GetDaySummaryDataForCampaign(string campaignIDXml, string subMediaType, bool loadEverything = true)
        {
            try
            {
                ...
                return FillIQAgentSummary(dsSSP);
            }
            catch (Exception ex)
            {
                Log4NetLogger.Error(ex);
        ...
        public AnalyticsDataModel GetHourSummaryDataForCampaign(string campaignIDXml, string subMediaType, bool loadEverything = true, string GroupByHeader = "")
        {
            try
            {
        ...
        public AnalyticsDataModel FillIQAgentSummary(DataSet dsSSP)
        {
                            ...
                            case "OverallSummary":
                                ...
                                foreach (DataRow row in dt.Rows)
                                {
                                    ...
                                    if (dt.Columns.Contains("DayDate") && !row["DayDate"].Equals(DBNull.Value))
                                    {
                                        ...
                                        summaryModel.DayTimeDisplay = string.Format("{0} {1}", summaryModel.SummaryDateTime.DayOfWeek, summaryModel.SummaryDateTime.ToShortTimeString());
                                        summaryModel.DayTimeID = string.Format("{0}_{1}", summaryModel.SummaryDateTime.DayOfWeek, summaryModel.SummaryDateTime.Hour);

                                        //Get Specific Day Part Data
                                        List<DayPartDataItem> dayPartItem = dayPartData == null ? new List<DayPartDataItem>() : dayPartData.Where(x => x.DayOfWeek == summaryModel.SummaryDateTime.DayOfWeek && summaryModel.SummaryDateTime.Hour == x.HourOfDay).ToList();
                                    ...
                                    if (dt.Columns.Contains("AM18_20") && !row["AM18_20"].Equals(DBNull.Value))
                                    {
                                        summaryModel.MaleAudience = 
                                            Convert.ToInt64(row["AM18_20"]) + Convert.ToInt64(row["AM21_24"]) + Convert.ToInt64(row["AM25_34"]) +
                                            Convert.ToInt64(row["AM35_49"]) + Convert.ToInt64(row["AM50_54"]) + Convert.ToInt64(row["AM55_64"]) + 
                                            Convert.ToInt64(row["AM65_Plus"]);
                                    }

                                    if (dt.Columns.Contains("AF18_20") && !row["AF18_20"].Equals(DBNull.Value))
                                    {
                                        summaryModel.FemaleAudience = 
                                            Convert.ToInt64(row["AF18_20"]) + Convert.ToInt64(row["AF21_24"]) + Convert.ToInt64(row["AF25_34"]) +
                                            Convert.ToInt64(row["AF35_49"]) + Convert.ToInt64(row["AF50_54"]) + Convert.ToInt64(row["AF55_64"]) + 
                                            Convert.ToInt64(row["AF65_Plus"]);
                                    }
