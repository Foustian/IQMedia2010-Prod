    ...
    if (_SearchMediumDesc) {
        ...
        switch (_SearchMedium) {
            case 'TV':
                ...
                $("#divTwitterTabContent").addClass('blurOnlyControls');
        ...
        $("#divTwitterTabContent").removeClass('blurOnlyControls');
        ...
        success: function (result) {
            if (result.isSuccess) {
                ...
                if (result.chartHTML != "") {
                    ...
                    JsonLineChart.xAxis.labels.formatter = FormatTopicsLabels;
...
function FormatTopicsLabels() {
    var text = this.value;
    var formatted = text.length > 25 ? text.substring(0, 25) + '...' : text;

    return '<div style="width:207px; overflow:hidden;" title="' + text + '">' + formatted + '</div>';
}
...
function OpenAdvanceSearchPopup() {
    ...
    else if ((_SearchMedium == '' || _SearchMedium == 'TW') && $("#divTWSetup").length > 0) {
...
function SetActiveAdvanceSearchValues(id) {
    ...
    if (AdvanceSearch != null) {
        ...
        if ((_SearchMedium == '' || _SearchMedium == 'TW') && $("#divTWSetup").length > 0) {
            if (AdvanceSearch.TwitterSettings != null) {
                $("#txtSearchTerm_TW").val(AdvanceSearch.TwitterSettings.SearchTerm);                                     
                $("#txtActor_TW").val(AdvanceSearch.TwitterSettings.TwitterHandle);

                if (AdvanceSearch.TwitterSettings.FollowersFrom != null) {
                    $("#txtFollowersCount_From_TW").val(AdvanceSearch.TwitterSettings.FollowersFrom);
                }
                if (AdvanceSearch.TwitterSettings.FollowersTo != null) {
                    $("#txtFollowersCount_To_TW").val(AdvanceSearch.TwitterSettings.FollowersTo);
                }

                if (AdvanceSearch.TwitterSettings.FollowingFrom != null) {
                    $("#txtFollowingCount_From_TW").val(AdvanceSearch.TwitterSettings.FollowingFrom);
                }
                if (AdvanceSearch.TwitterSettings.FollowingTo != null) {
                    $("#txtFollowingCount_To_TW").val(AdvanceSearch.TwitterSettings.FollowingTo);
                }

                if (AdvanceSearch.TwitterSettings.KloutScoreFrom != null) {
                    $("#txtKloutScore_From_TW").val(AdvanceSearch.TwitterSettings.KloutScoreFrom);
                }
                if (AdvanceSearch.TwitterSettings.KloutScoreTo != null) {
                    $("#txtKloutScore_To_TW").val(AdvanceSearch.TwitterSettings.KloutScoreTo);
                }
                var output = $.map(AdvanceSearch.TwitterSettings.ExcludeHandleList, function (obj, index) { return obj; }).join('; ');
                $("#txtExcludeHandles_TW").val(output);
            }
            else {
                AdvanceSearch.TwitterSettings = new Object();
                AdvanceSearch.TwitterSettings.TwitterHandle = null;
                AdvanceSearch.TwitterSettings.FollowersFrom = null;
                AdvanceSearch.TwitterSettings.FollowersTo = null;
                AdvanceSearch.TwitterSettings.FollowingFrom = null;
                AdvanceSearch.TwitterSettings.FollowingTo = null;
                AdvanceSearch.TwitterSettings.KloutScoreFrom = null;
                AdvanceSearch.TwitterSettings.KloutScoreTo = null;
                AdvanceSearch.TwitterSettings.ExcludeHandleList = [];
            }
        }
...
function OnLoadAgentAdvancedSearchSuccess(result) {
    ...
    if (result.isSuccess && result.agentAdvancedSearch != null) {
        ...
        if (searchRequestObject.Twitter != null && searchRequestObject.TwitterSpecified == true) {
            ...
            //Search Term
            if (searchRequestObject.Twitter.SearchTerm.SearchTerm != null) {
                AdvanceSearch.TwitterSettings.SearchTerm = searchRequestObject.Twitter.SearchTerm.SearchTerm.trim();
            }

            //Twitter Handle
            if (searchRequestObject.Twitter.Actor != null) {
                AdvanceSearch.TwitterSettings.TwitterHandle = searchRequestObject.Twitter.Actor.trim().toLowerCase();
            }

            //Followers Range
            if (searchRequestObject.Twitter.ActorFriendsRange != null) {
                AdvanceSearch.TwitterSettings.FollowersFrom = searchRequestObject.Twitter.ActorFriendsRange.From;
                AdvanceSearch.TwitterSettings.FollowersTo = searchRequestObject.Twitter.ActorFriendsRange.To;
            }

            //Following Range
            if (searchRequestObject.Twitter.ActorFollowersRange != null) {
                AdvanceSearch.TwitterSettings.FollowingFrom = searchRequestObject.Twitter.ActorFollowersRange.From;
                AdvanceSearch.TwitterSettings.FollowingTo = searchRequestObject.Twitter.ActorFollowersRange.To;
            }

            //Klout Score Range
            if (searchRequestObject.Twitter.KloutScoreRange != null) {
                AdvanceSearch.TwitterSettings.KloutScoreFrom = searchRequestObject.Twitter.KloutScoreRange.From;
                AdvanceSearch.TwitterSettings.KloutScoreTo = searchRequestObject.Twitter.KloutScoreRange.To;
            }

            //Exclude Domains
            if (searchRequestObject.Twitter.ExclusionHandles != null) {
                AdvanceSearch.TwitterSettings.ExcludeHandleList = searchRequestObject.Twitter.ExclusionHandles;
            }
        }    
...
function isDefaultAdvancedSearch(AdvanceSearch) {
    if (AdvanceSearch != null) {
        ...
        //Twitter
        if (AdvanceSearch.TwitterSettings != null 
        && ((AdvanceSearch.TwitterSettings.SearchTerm != null && AdvanceSearch.TwitterSettings.SearchTerm.trim() != '')
        || (AdvanceSearch.TwitterSettings.TwitterHandle != null && AdvanceSearch.TwitterSettings.TwitterHandle.trim() != '')
        || (AdvanceSearch.TwitterSettings.FollowersFrom != null && AdvanceSearch.TwitterSettings.FollowersFrom.trim() != '')
        || (AdvanceSearch.TwitterSettings.FollowersTo != null && AdvanceSearch.TwitterSettings.FollowersTo.trim() != '')
        || (AdvanceSearch.TwitterSettings.FollowingFrom != null && AdvanceSearch.TwitterSettings.FollowingFrom.trim() != '')
        || (AdvanceSearch.TwitterSettings.FollowingTo != null && AdvanceSearch.TwitterSettings.FollowingTo.trim() != '')
        || (AdvanceSearch.TwitterSettings.KloutScoreFrom != null && AdvanceSearch.TwitterSettings.KloutScoreFrom.trim() != '')
        || (AdvanceSearch.TwitterSettings.KloutScoreTo != null && AdvanceSearch.TwitterSettings.KloutScoreTo.trim() != '')
        || (AdvanceSearch.TwitterSettings.ExcludeHandleList != null && AdvanceSearch.TwitterSettings.ExcludeHandleList.length > 0))) {
            return false;
        }
...
function SetAdvanceSearch(ID) {
    if (IsValidSearch()) {
        if (IsNewSearch(ID)) {
            ...
            if ((_SearchMedium == '' || _SearchMedium == 'TW') && $("#divTWSetup").length > 0) {
                AdvanceSearch.TwitterSettings = new Object();
                AdvanceSearch.TwitterSettings.SearchTerm = $("#txtSearchTerm_TW").val().trim();
                AdvanceSearch.TwitterSettings.TwitterHandle = $("#txtActor_TW").val().trim().toLowerCase();
                if ($("#txtFollowersCount_From_TW").val().trim().toLowerCase() != "") AdvanceSearch.TwitterSettings.FollowersFrom = $("#txtFollowersCount_From_TW").val().trim().toLowerCase();
                if ($("#txtFollowersCount_To_TW").val().trim().toLowerCase() != "") AdvanceSearch.TwitterSettings.FollowersTo = $("#txtFollowersCount_To_TW").val().trim().toLowerCase();

                if ($("#txtFollowingCount_From_TW").val().trim().toLowerCase() != "") AdvanceSearch.TwitterSettings.FollowingFrom = $("#txtFollowingCount_From_TW").val().trim().toLowerCase();
                if ($("#txtFollowingCount_To_TW").val().trim().toLowerCase() != "") AdvanceSearch.TwitterSettings.FollowingTo = $("#txtFollowingCount_To_TW").val().trim().toLowerCase();

                if ($("#txtKloutScore_From_TW").val().trim().toLowerCase() != "") AdvanceSearch.TwitterSettings.KloutScoreFrom = $("#txtKloutScore_From_TW").val().trim().toLowerCase();
                if ($("#txtKloutScore_To_TW").val().trim().toLowerCase() != "") AdvanceSearch.TwitterSettings.KloutScoreTo = $("#txtKloutScore_To_TW").val().trim().toLowerCase();

                AdvanceSearch.TwitterSettings.ExcludeHandleList = [];
                $.each($("#txtExcludeHandles_TW").val().trim().split(";"), function () {
                    if ($.trim(this) != '') {
                        AdvanceSearch.TwitterSettings.ExcludeHandleList.push($.trim(this).toLowerCase());
                    }
                });
            }
...
function IsNewSearch(ID) {
    ...
    if (AdvanceSearch != null) {
        ...
        if ((_SearchMedium == '' || _SearchMedium == 'TW') && $("#divTWSetup").length > 0) {
            ...
            $.each($("#txtExcludeHandles_TW").val().trim().split(";"), function () {
                if ($.trim(this) != '') {
                    TWExcludeList.push($.trim(this).toLowerCase());
                }
            });

            if ((AdvanceSearch.TwitterSettings.SearchTerm != $("#txtSearchTerm_TW").val().trim())
            || (AdvanceSearch.TwitterSettings.TwitterHandle != $("#txtActor_TW").val().trim().toLowerCase())
            || (AdvanceSearch.TwitterSettings.FollowersFrom != $("#txtFollowersCount_From_TW").val().trim().toLowerCase())
            || (AdvanceSearch.TwitterSettings.FollowersTo != $("#txtFollowersCount_To_TW").val().trim().toLowerCase())
            || (AdvanceSearch.TwitterSettings.FollowingFrom != $("#txtFollowingCount_From_TW").val().trim().toLowerCase())
            || (AdvanceSearch.TwitterSettings.FollowingTo != $("#txtFollowingCount_To_TW").val().trim().toLowerCase())
            || (AdvanceSearch.TwitterSettings.KloutScoreFrom != $("#txtKloutScore_From_TW").val().trim().toLowerCase())
            || (AdvanceSearch.TwitterSettings.KloutScoreTo != $("#txtKloutScore_To_TW").val().trim().toLowerCase())
            || ($(AdvanceSearch.TwitterSettings.ExcludeHandleList).not(TWExcludeList).length != 0 || $(TWExcludeList).not(AdvanceSearch.TwitterSettings.ExcludeHandleList).length != 0)) {
                isNew = true;
            }
        }
