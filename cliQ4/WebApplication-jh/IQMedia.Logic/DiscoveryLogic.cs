...
//End Criteria
        ...
        public DiscoverySearchResponse SearchTwitter(string searchTerm, string searchTermName, DateTime? fromDate, DateTime? toDate, string medium, string p_fromRecordID, string pmgSearchUrl, TwitterAdvanceSearchSettings TwitterSearchSettings)
        {
            try
            {
                string updatedSearchTerm = (TwitterSearchSettings == null || string.IsNullOrWhiteSpace(TwitterSearchSettings.SearchTerm)) ? searchTerm : TwitterSearchSettings.SearchTerm.Trim();
                ...
                if (!string.IsNullOrWhiteSpace(ConfigurationManager.AppSettings["GnipTag"]))
                {
                    searchTwitterRequest.gnip_tag = ConfigurationManager.AppSettings["GnipTag"].Split(',').Select(a => new Guid(a.Trim())).ToList();
                }
                if (!string.IsNullOrWhiteSpace(medium))
                {
                ...
                else
                {
                    ...
                    searchTwitterRequest.FacetRangeEnds = toDate;
                }
...
//Criteria
                searchTwitterRequest.SearchTerm = updatedSearchTerm;

                if (TwitterSearchSettings != null)
                {
                    searchTwitterRequest.ActorDisplayName = TwitterSearchSettings.TwitterHandle;

                    if (TwitterSearchSettings.FollowingFrom != null) searchTwitterRequest.FriendsRangeFrom = Int64.Parse(TwitterSearchSettings.FollowingFrom);
                    if (TwitterSearchSettings.FollowingTo != null) searchTwitterRequest.FriendsRangeTo = Int64.Parse(TwitterSearchSettings.FollowingTo);

                    if (TwitterSearchSettings.FollowersFrom != null) searchTwitterRequest.FollowersRangeFrom = Int64.Parse(TwitterSearchSettings.FollowersFrom);
                    if (TwitterSearchSettings.FollowersTo != null) searchTwitterRequest.FollowersRangeTo = Int64.Parse(TwitterSearchSettings.FollowersTo);

                    if (TwitterSearchSettings.KloutScoreFrom != null) searchTwitterRequest.KloutRangeFrom = Int64.Parse(TwitterSearchSettings.KloutScoreFrom);
                    if (TwitterSearchSettings.KloutScoreTo != null) searchTwitterRequest.KloutRangeTo = Int64.Parse(TwitterSearchSettings.KloutScoreTo);
                    
                    searchTwitterRequest.ExcludeHandles = TwitterSearchSettings.ExcludeHandleList;
                }
//End Criteria
...
//End Criteria
        ...
        public List<PieChartResponse> PieChart(List<DiscoverySearchResponse> lstDiscoverySearchResponse, List<DiscoverySearchResponse> lstDiscoverySearchResponseFeedClass, string[] searchTerm, string medium, bool p_Isv4TV, bool p_Isv4NM, bool p_Isv4SM, bool p_Isv4TW)
        {
            try
            {
                ...
                piechart.paletteColors = "394900,15335D,005E8F,FFB451,448FF2,7A045C";
                ...
                foreach (string sTerm in searchTerm)
                {
                    ...
                    {
                        PieChartdata pieChartdataTV = new PieChartdata();
                        pieChartdataTV.label = CommonFunctions.CategoryType.TV.ToString();
                        pieChartdataTV.value = lstDiscoverySearchResponse.Where(w => w.MediumType.Equals(CommonFunctions.CategoryType.TV.ToString()) && w.SearchTerm.Equals(sTerm)).GroupBy(g => g.MediumType)
                                                                                .Select(s =>
                                                                                    Convert.ToInt64(s.Sum(s1 => Convert.ToInt64(s1.TotalResult)))).FirstOrDefault();

                        lstPieChartdata.Add(pieChartdataTV);
                    }
                    ...
                    {
                        PieChartdata pieChartdataNM = new PieChartdata();
                        pieChartdataNM.label = CommonFunctions.GetEnumDescription(CommonFunctions.CategoryType.NM);
                        pieChartdataNM.value = lstDiscoverySearchResponse.Where(w => w.MediumType.Equals(CommonFunctions.CategoryType.NM.ToString()) && w.SearchTerm.Equals(sTerm)).GroupBy(g => g.MediumType)
                                                                                .Select(s =>
                                                                                    Convert.ToInt64(s.Sum(s1 => Convert.ToInt64(s1.TotalResult)))).FirstOrDefault();

                        lstPieChartdata.Add(pieChartdataNM);
                    }

                    // Get TW Data
                    if (p_Isv4TW && (string.IsNullOrWhiteSpace(medium) || medium == CommonFunctions.CategoryType.TW.ToString()))
                    {
                        PieChartdata pieChartdataTW = new PieChartdata();
                        pieChartdataTW.label = CommonFunctions.GetEnumDescription(CommonFunctions.CategoryType.TW);
                        pieChartdataTW.value = lstDiscoverySearchResponse.Where(w => w.MediumType.Equals(CommonFunctions.CategoryType.TW.ToString()) && String.Compare(w.SearchTerm, sTerm, true) == 0).GroupBy(g => g.MediumType)
                                                                                .Select(s =>
                                                                                   Convert.ToInt64(s.Sum(s1 => Convert.ToInt64(s1.TotalResult)))).FirstOrDefault();

                        lstPieChartdata.Add(pieChartdataTW);
                    }
                        ...
                        medium == CommonFunctions.CategoryType.Forum.ToString()))
                    {
                        ...
                        {
                            PieChartdata pieChartdataSM = new PieChartdata();
                            pieChartdataSM.label = "Social Media";
                            pieChartdataSM.value = 0;

                            PieChartdata pieChartdataBlog = new PieChartdata();
                            pieChartdataBlog.label = "Blog";
                            pieChartdataBlog.value = 0;

                            PieChartdata pieChartdataForum = new PieChartdata();
                            pieChartdataForum.label = "Forum";
                            pieChartdataForum.value = 0;

                            lstPieChartdata.Add(pieChartdataSM);
                            lstPieChartdata.Add(pieChartdataBlog);
                            lstPieChartdata.Add(pieChartdataForum);
                        }
                        else
                        {
                            lstPieChartdata.AddRange(totalRecords.Where(w => w.SearchTerm.Equals(sTerm)).Select(s => new PieChartdata()
                            {
                                label = s.FeedClass,
        ...
        public string HighChartsLineChart(List<DiscoverySearchResponse> lstDiscoverySearchResponse, Boolean isHourData, decimal p_ClientGmtOffset, decimal p_ClientDstOffset)
        {
            try
            {
                ...
                */
                ...
                highLineChartOutput.hChart = new HChart() { height = 370, width = 750, type = "spline" };
            ...
            try
            {
                ...
                */
                ...
                foreach (string sTerm in searchTerms)
                {
                    ...
                    // Get Twitter Data
                    if (isv4TW && (string.IsNullOrWhiteSpace(medium) || medium == CommonFunctions.CategoryType.TW.ToString()))
                    {
                        series = new Series();
                        series.name = CommonFunctions.GetEnumDescription(CommonFunctions.CategoryType.TW);
                        series.data = totalRecords.Where(w => w.FeedClass.Equals(CommonFunctions.CategoryType.TW.ToString()) && w.SearchTerm.Equals(sTerm))
                                                                    .Select(s => new HighChartDatum()
                                                                    {
                                                                        y = s.TotalRecords,
                                                                        SearchTerm = sTerm,
                                                                        Type = CommonFunctions.CategoryType.TW.ToString()
                                                                    }).ToList();
                        lstSeries.Add(series);
                    }
        ...
        public Dictionary<string, object> HighChartsPieChartBySearchTerm(List<DiscoverySearchResponse> lstDiscoverySearchResponse, List<DiscoverySearchResponse> lstDiscoverySearchResponseFeedClass, string[] searchTerm, string[] searchName, string medium, bool p_Isv4TV, bool p_Isv4NM, bool p_Isv4SM, bool p_Isv4TW, bool p_Isv4PQ)
        {
            try
            {
                ...
                for (int i = 0; i < searchTerm.Length; i++ )
                {
                    ...
                    if (p_Isv4TW && (string.IsNullOrWhiteSpace(medium) || medium == CommonFunctions.CategoryType.TW.ToString()))
                    {
                        dictCounts[sName] += lstDiscoverySearchResponse.Where(w => w.MediumType.Equals(CommonFunctions.CategoryType.TW.ToString()) && String.Compare(w.SearchTerm, sTerm, true) == 0).GroupBy(g => g.SearchTerm)
                                                                                .Select(s =>
                                                                                   Convert.ToInt64(s.Sum(s1 => Convert.ToInt64(s1.TotalResult)))).FirstOrDefault();
                    }
        ...
        public List<PieChartResponse> HighChartsPieChartByMedium(List<DiscoverySearchResponse> lstDiscoverySearchResponse, List<DiscoverySearchResponse> lstDiscoverySearchResponseFeedClass, string[] searchTerm, string[] searchName, string medium, bool p_Isv4TV, bool p_Isv4NM, bool p_Isv4SM, bool p_Isv4TW, bool p_Isv4PQ)
        {
            try
            {
...
//End Criteria
        ...
        public List<DiscoveryMediaResult> SearchTwitterResult(int p_CustomerKey, string searchTerm, DateTime? fromDate, DateTime? toDate, string medium, bool isAsc, IQClient_ThresholdValueModel p_IQClient_ThresholdValueModel, Int32 p_PageSize, string pmgSearchUrl, TwitterAdvanceSearchSettings TwitterSearchSettings)//Int64 startRecordID, string fromRecordID,
        {
            ...
            try
            {
                ...
                Log4NetLogger.Debug("Task: SearchTwitterResult -- searchTerm :" + updatedSearchTerm);
                bool isError = false;
                System.Uri PMGSearchRequestUrl = new Uri(pmgSearchUrl);
                SearchEngine searchEngine = new SearchEngine(PMGSearchRequestUrl);
                SearchTwitterRequest searchTwitterRequest = new SearchTwitterRequest();
                ...
                else
                {
                    ...
                    searchTwitterRequest.EndDate = toDate;
                }
...
//End Criteria
                ...
                UtilityLogic.InsertPMGSearchLog(p_CustomerKey, searchTwitterRequest.SearchTerm, string.Empty, string.Empty, string.Empty, string.Empty, searchTwitterRequest.PageNumber, searchTwitterRequest.PageSize, 0, searchTwitterRequest.StartDate, searchTwitterRequest.EndDate, IQMedia.Shared.Utility.CommonFunctions.SearchType.Discovery_TW.ToString(), _Responce);


                #endregion

                if (searchTwitterResult != null && searchTwitterResult.TwitterResults != null)
                {
                    foreach (TwitterResult twitterResult in searchTwitterResult.TwitterResults)
                    {
                        ...
                        discoveryMediaResult.TwitterTweetID = twitterResult.tweet_id;
...
//End Criteria
        ...
        public DiscoveryTopicsResponse GetTwitterTopics(string searchTerm, DateTime fromDate, DateTime toDate, string pmgSearchUrl, TwitterAdvanceSearchSettings twitterSettings, CancellationToken token, DiscoveryTopicsResponse dtr)
        {
            try
            {
                // Build request XML
                ...
                XElement root = new XElement("parameters",
                                    new XElement("twitterbaseurl", pmgSearchUrl),
                                    new XElement("searchterm", searchTerm),
                                    new XElement("date", "[" + fromDate.ToString("yyyy-MM-dd'T'HH:mm:ss'Z'") + " TO " + toDate.ToString("yyyy-MM-dd'T'HH:mm:ss'Z'") + "]"),
                                    new XElement("actor"),
                                    new XElement("excludeactor"),
                                    new XElement("resultnum", "10")
                                );

                if (twitterSettings != null)
                {
                    if (!String.IsNullOrWhiteSpace(twitterSettings.TwitterHandle))
                    {
                        root.Descendants("actor").FirstOrDefault().Value = twitterSettings.TwitterHandle;
                    }
                    if (twitterSettings.ExcludeHandleList != null && twitterSettings.ExcludeHandleList.Count > 0)
                    {
                        root.Descendants("excludeactor").FirstOrDefault().Value = "(" + String.Join(" OR ", twitterSettings.ExcludeHandleList) + ")";
                    }
                }
            ...
            {
                Log4NetLogger.Error("GetTwitterTopics: " + ex.Message + " || " + ex.StackTrace);
                dtr.IsValid = false;
            }
            return dtr;
        }
        ...
        public HighColumnChartModel HighChartsTopicsBarChart(List<Topics> lstTopics, string searchTerm, string searchName, string medium, string mediumName)
        {
            ...
            highColumnChartModel.xAxis = new XAxis()
            {
                ...
                labels = new labels()
                {
                    ...
                    formatter = "FormatTopicsLabels" // Indicates use of formatter. Actual function is set in Discovery.js GetTopics().
                }
            };
