...
namespace IQMedia.Web.Logic
{
    public class DiscoveryLiteLogic : IQMedia.Web.Logic.Base.ILogic
    {
        ...
        public DiscoveryLiteSearchResponse SearchTwitter(string searchTerm, DateTime? fromDate, DateTime? toDate, string medium, string tvMarket, string p_fromRecordID, string pmgSearchUrl)
        {
            try
            {
                Boolean isError = false;
                ...
                if (!string.IsNullOrEmpty(ConfigurationManager.AppSettings["GnipTag"]))
                {
        ...
        public List<PieChartResponse> PieChart(List<DiscoveryLiteSearchResponse> lstDiscoverySearchResponse, List<DiscoveryLiteSearchResponse> lstDiscoverySearchResponseFeedClass,
        ...
        public Dictionary<string, object> HighChartsPieChartBySearchTerm(List<DiscoveryLiteSearchResponse> lstDiscoverySearchResponse, List<DiscoveryLiteSearchResponse> lstDiscoverySearchResponseFeedClass,
                                                            string[] searchTerm, string medium,
                                                            bool p_Isv4TV, bool p_Isv4NM,
                                                            bool p_Isv4SM, bool p_Isv4TW, bool p_Isv4PQ)
        {
            try
            {
                ...
                foreach (string sTerm in searchTerm)
                {
                    ...
                    if (p_Isv4TW && (string.IsNullOrWhiteSpace(medium) || medium == CommonFunctions.CategoryType.TW.ToString()))
                    {
                        dictCounts[sTerm] += lstDiscoverySearchResponse.Where(w => w.MediumType.Equals(CommonFunctions.CategoryType.TW.ToString()) && String.Compare(w.SearchTerm, sTerm, true) == 0).GroupBy(g => g.SearchTerm)
                                                                                .Select(s =>
                                                                                   Convert.ToInt64(s.Sum(s1 => Convert.ToInt64(s1.TotalResult)))).FirstOrDefault();
                    }
