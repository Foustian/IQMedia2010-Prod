...
namespace IQMedia.Web.Logic
{
    public class DashboardLogic : IQMedia.Web.Logic.Base.ILogic
    {
        ...
        public List<IQAgent_DaySummaryModel> GetGoogleDataByHour(Guid clientGuid, DateTime fromDate, DateTime toDate)
        {
            DashboardDA dashboardDA = new DashboardDA();

            List<IQAgent_DaySummaryModel> lstIQAgent_DaySummaryModel = dashboardDA.GetGoogleDataByHour(clientGuid, fromDate, toDate);

            return lstIQAgent_DaySummaryModel;
        }

        public List<IQAgent_DaySummaryModel> GetGoogleDataByDay(Guid clientGuid, DateTime fromDate, DateTime toDate)
        {
            DashboardDA dashboardDA = new DashboardDA();

            List<IQAgent_DaySummaryModel> lstIQAgent_DaySummaryModel = dashboardDA.GetGoogleDataByDay(clientGuid, fromDate, toDate);

            return lstIQAgent_DaySummaryModel;
        }

        public List<IQAgent_DaySummaryModel> GetGoogleDataByMonth(Guid clientGuid, DateTime fromDate, DateTime toDate)
        {
            DashboardDA dashboardDA = new DashboardDA();

            List<IQAgent_DaySummaryModel> lstIQAgent_DaySummaryModel = dashboardDA.GetGoogleDataByMonth(clientGuid, fromDate, toDate);
            ...
            try
            {
                ...
                foreach (var subMedia in lstMediaCategories)
                {
                        ...
                        ((p_Isv4BLPMAccess || p_IsV4PQAccess) && subMedia == CommonFunctions.DashBoardMediumType.PM)) // BLPM and ProQuest data is combined
                    {
                        ...
                        string enumDesc = CommonFunctions.GetEnumDescription(subMedia);
                        Int64 sumNumDocs = numOfDocs != null ? numOfDocs : 0;
                        lstObject.Add(new object[] { enumDesc + " " + sumNumDocs.ToString("N0"), Convert.ToInt64(numOfDocs != null ? numOfDocs : 0)});
        ...
        public SummaryReportMulti HighChartsLineChart(List<SummaryReportModel> listOfSummaryReportData, DateTime p_FromDate, DateTime p_ToDate, int? chartWidth, bool p_Isv4TMAccess, List<string> p_SearchRequestIDs, List<IQAgent_ComparisionValues> p_ListOfIQAgent_ComparisionValues,
            bool p_Isv4NMAccess, bool p_Isv4SMAccess, bool p_Isv4TWAccess, bool p_Isv4TVAccess, bool p_Isv4BLPMAccess, bool p_Isv4UGCAccess, bool p_Isv4PQAccess, bool p_NielsenAccess, bool p_CompeteDataAccess, bool p_Isv4GoogleAccess)
        {
            try
            {
                List<DateTime> dateRange = new List<DateTime>();
                ...
                */
                ...
                // Create series for Google Analytics data
                if (p_Isv4GoogleAccess)
                {
                    highLineChartSubMediaOutput.yAxis.Add(new YAxis() { title = new Title2() { text = "Google Analytics", rotation = 90 }, opposite = true });

                    List<string> lstGoogleMetrics = listOfSummaryReportData.Where(w => w.MediaType == CommonFunctions.DashBoardMediumType.Google.ToString()).Select(s => s.Query_Name).Distinct().ToList();
                    foreach (string metric in lstGoogleMetrics)
                    {
                        Series series = new Series();
                        series.data = new List<HighChartDatum>();
                        series.name = metric;
                        ...
                        // loop for each date to create list of data for selected metric
                        foreach (var item in dateRange)
                        {
                            var daywiseSum = listOfSummaryReportData.Where(smr => String.Compare(smr.Query_Name, metric, true) == 0 && smr.GMT_DateTime.Equals(item)).Sum(s => s.Number_Docs);
                            ...
                            */
                            HighChartDatum highChartDatum = new HighChartDatum();
                            highChartDatum.y = daywiseSum != null ? daywiseSum : 0;
                            highChartDatum.Value = metric;
                            highChartDatum.Type = "Google";
                            series.data.Add(highChartDatum);
                        }
                ...
                jsonResult = CommonFunctions.SearializeJson(highLineChartSubMediaOutput);
                lstSummaryReportMulti.SubMediaRecords = jsonResult;
        ...
        public SummaryReportMulti HighChartsLineChartMonth(List<SummaryReportModel> listOfSummaryReportData, DateTime p_FromDate, DateTime p_ToDate, bool p_Isv4TMAccess, List<string> p_SearchRequestIDs, List<IQAgent_ComparisionValues> p_ListOfIQAgent_ComparisionValues,
            bool p_Isv4NMAccess, bool p_Isv4SMAccess, bool p_Isv4TWAccess, bool p_Isv4TVAccess, bool p_Isv4BLPMAccess, bool p_Isv4UGCAccess, bool p_Isv4PQAccess, bool p_NielsenAccess, bool p_CompeteDataAccess, bool p_Isv4GoogleAccess)
        {
            try
            {
                ...
                if (p_SearchRequestIDs != null && p_SearchRequestIDs.Count > 0)
                {
                    ...
                    foreach (var searchRequest in p_SearchRequestIDs)
                    {
                        ...
                        if (SearchRequest != null)
                        {
                            ...
                            foreach (var item in dateRange)
                            {
                                var daywiseSum = listOfSummaryReportData.Where(smr => String.Compare(smr.SearchRequestID.ToString(), searchRequest, true) == 0 && smr.GMT_DateTime.Month.Equals(item.Month) && smr.GMT_DateTime.Year.Equals(item.Year)
                                        && (
                                            (p_Isv4TMAccess && smr.SubMediaType == CommonFunctions.DashBoardMediumType.Radio.ToString()) ||
                                            (p_Isv4NMAccess && smr.SubMediaType == CommonFunctions.DashBoardMediumType.NM.ToString()) ||
                                            (p_Isv4SMAccess && smr.SubMediaType == CommonFunctions.DashBoardMediumType.SocialMedia.ToString()) ||
                ...
                */
                ...
                if (p_Isv4GoogleAccess)
                {
                    ...
                    foreach (string metric in lstGoogleMetrics)
                    {
                        ...
                        foreach (var item in dateRange)
                        {
                            var daywiseSum = listOfSummaryReportData.Where(smr => String.Compare(smr.Query_Name, metric, true) == 0 && smr.GMT_DateTime.Month.Equals(item.Month) && smr.GMT_DateTime.Year.Equals(item.Year)).Sum(s => s.Number_Docs);
        ...
        public SummaryReportMulti HighChartsLineChartHour(List<SummaryReportModel> listOfSummaryReportData, DateTime p_FromDate, DateTime p_ToDate, int? chartWidth, bool p_Isv4TMAccess, decimal p_ClientGmtOffset, decimal p_ClientDstOffset, List<string> p_SearchRequestIDs, List<IQAgent_ComparisionValues> p_ListOfIQAgent_ComparisionValues,
            bool p_Isv4NMAccess, bool p_Isv4SMAccess, bool p_Isv4TWAccess, bool p_Isv4TVAccess, bool p_Isv4BLPMAccess, bool p_Isv4UGCAccess, bool p_Isv4PQAccess, bool p_NielsenAccess, bool p_CompeteDataAccess, bool p_Isv4GoogleAccess)
        {
            try
            {
