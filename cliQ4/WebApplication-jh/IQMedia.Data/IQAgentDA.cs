    ...
    public class IQAgentDA : IDataAccess
    {
        ...
        public void GetSearchTermByIQAgentTVResultID(Guid clientGuid, Int64 iqagentTVResultID, out Guid rlVideoGUID, out string searchTerm)
        {
            try
            {
                ...
                IDataReader reader = DataAccess.GetDataReader("usp_v4_IQAgent_SearchRequest_SelectSearchTermBy_IQAgent_TVResultsID", dataTypeList);
        ...
        public int UpdateIsRead(Guid clientGuid, string mediaIDXml, bool isRead)
        {
            try
            {
                List<DataType> dataTypeList = new List<DataType>();
                dataTypeList.Add(new DataType("@ClientGUID", DbType.Guid, clientGuid, ParameterDirection.Input));
                dataTypeList.Add(new DataType("MediaIDXml", DbType.Xml, mediaIDXml, ParameterDirection.Input));
                dataTypeList.Add(new DataType("@IsRead", DbType.Boolean, isRead, ParameterDirection.Input));

                string rowsAffected = Convert.ToString(DataAccess.ExecuteScalar("usp_v4_IQAgent_MediaResults_UpdateIsRead", dataTypeList));
