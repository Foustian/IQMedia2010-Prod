...
namespace IQMedia.Web.Logic
{
    public class AnalyticsLogic: IQMedia.Web.Logic.Base.ILogic
    {
        ...
        public AnalyticsDataModel GetHourSummaryData(Guid clientGUID, string searchRequestXml, string subMediaType, decimal gmtAdjustment, decimal dstAdjustment, bool loadEverything = true, string GroupByHeader = "")
        {
            AnalyticsDA analyticsDA = new AnalyticsDA();

            AnalyticsDataModel dataModel = analyticsDA.GetHourSummaryData(clientGUID, searchRequestXml, subMediaType, gmtAdjustment, dstAdjustment, loadEverything, GroupByHeader);
            return dataModel;
        }
        ...
        public AnalyticsDataModel GetDaySummaryData(Guid clientGUID, string searchRequestXml, string subMediaType, decimal gmtAdjustment, decimal dstAdjustment, bool loadEverything = true)
        {
            AnalyticsDA analyticsDA = new AnalyticsDA();

            AnalyticsDataModel dataModel = analyticsDA.GetDaySummaryData(clientGUID, searchRequestXml, subMediaType, gmtAdjustment, dstAdjustment, loadEverything);
            return dataModel;
        }
        ...
        public Dictionary<string, object> GetChart(AnalyticsRequest graphRequest, AnalyticsSecondaryTable table, AnalyticsDataModel analyticsData, Dictionary<string, string> allGroups, List<IQ_MediaTypeModel> subMediaTypes)
        {
            try
            {
                Stopwatch sw = new Stopwatch();
                ...
                if (graphRequest.Tab != SecondaryTabID.Demographic)
                {
                    ...
                    analyticsData.SummaryDataList = analyticsData.SummaryDataList.Where(w => groupByPI.GetValue(w, null) != null).ToList();

                    foreach (var group in allGroups)
                    {
                        var groupSummaries = analyticsData.SummaryDataList.Where(w => groupByPI.GetValue(w, null).ToString() == ((graphRequest.Tab == SecondaryTabID.Networks || graphRequest.Tab == SecondaryTabID.Shows) ? group.Value : group.Key)).ToList();
                ...
                else
                {
                    groupings.Add(new AnalyticsGrouping() {
                        ID = "demographic",
                        Name = "demographic",
                        Summaries = analyticsData.SummaryDataList
                    });
                }
        ...
        public string CreateOverlay(List<AnalyticsGrouping> groupings, AnalyticsRequest request, int overlayType)
        {
            try
            {
                Stopwatch sw = new Stopwatch();
