/**
   ...
   TODO:
   See Google Doc for current player issues, requests, etc.

   CHANGELOG:
   
   2.0.8.96 Added ability to Play/Pause the player from FLEX container.
   2.0.8.95 Fixed issue where if AdapTV failed to load, the clip didn't play
   2.0.8.94 Bug fixes related to no ads not auto play and decoupling of menu data load before playback
   2.0.8.93 Decoupled the menu metadata load from the video playback/adaptv fetch
   2.0.8.92 Fix volume off injection for audio only clips - oops, couple other fixes
   2.0.8.91	close menus, and popups when loadclip is called
   
   2.0.8.90	Added pid for loadClip pass in
   2.0.8.89 Fixed subsequent missing pre-rolls in widget mode - other adjustments
   
   2.0.8.88 Changes to enhance pre-roll - exeuction order NO video before ADS - ughhh....
   2.0.8.87 Leverage undocumented info from AdapTV to handle detection of Pre-roll video presence - no longer need preRoll node from getVars XML
   2.0.8.86	Re-arrange code execution order, functionality, to support pre-roll video, post-roll ads
   2.0.8.85	Moved to Flash CS5 XFL file format for .FLA and Flash Builder for class editing
   2.0.8.84	Merged changes for handling AdapTV context object from 2.0.8.82 fork from BEVANS with 2.0.8.83 form from RHALL
   2.0.8.83	Removed componentshim and UILoader components to get rid of focusmanager issues to fix focusmanager issues with 3rd party ad control googleIMA.swf
   Rewrote station thumbnail loading to use standard loader instead of uiloader to fix focusmanager issues with 3rd party ad control googleIMA.swf
   2.0.8.82	Added pause event for click thru on Ads
   Added AS3 CONFIG Constants for easy production/development compilation
...
package com.redlasso.core
{
	...
	import tv.adap.proxy.AdaptvPlayer3;
	...
	public class RedlassoPlayer extends MovieClip
	{

		/* START ADAPTV */
		private var adaptvPlayer3:AdaptvPlayer3; // = new AdaptvPlayer3(this);

		private var adaptvConfig:Object = new Object();

		private var videoStatus:Object = new Object();

		private var videoStatus_done:Boolean = false;
		...
		// Use Flash IDE AS3 config constants - toggle values of PRODUCTION and DEVELOPMENT to adjust build output - DO NOT CHANGE BELOW

		CONFIG::PRODUCTION
		{
			public static const srvString:String = "http://services.redlasso.com";

			private static const verPP:String = "P";
		}
		...
		CONFIG::DEVELOPMENT
		{
			...
			private static const verPP:String = "D";
		}

		// Application version and build info - ADJUST HERE for version only - D/P handled above via config constants
		//private static const versionBuild:String = "2.0.8.96" + verPP + " RMH";
		private static const versionBuild:String = "1.0";
		...
		// Values used for tacking on extra info to service calls to test different types of failures - used for debugging only - should be "" for standard production/testing
		...
		public function RedlassoPlayer():void
		{
			if (stage)
			{
				oneTimeInit();
		...
		public function oneTimeInit(event:Event=null):void
		{
			...
			initClip="a67d68f8-2c08-4df5-a00e-9b320795e2c3";
			trace (initClip);
			...
			/* START ADAPTV */
			videoStatus.playBackReady=false;
			videoStatus.firstPass=true;
			videoStatus.adsEnabled=false;
			videoStatus.ready=false;
			videoStatus.startAds=true;
			videoStatus.APILoaded = false;
			/* END ADAPTV */
		 ...
		 * handles resetting certain display objects, adapTV.destroy to clear out ads
		 ...
		 */
		public function LoadClip(id:String, playStyle:String, partnerID:String="")
		{
			pid = partnerID;
			videoStatus.reloading = true;
			closeMenu();
			closeSubMenu();
			mcPlay.enabled=true;
			mcPlay.visible=false;
			...
			hasVideo="NULL";
			initClip=id;
			videoStatus.preRollBreak = "BREAK_NULL";
			...
			{
				if (firstTimeLoadClip != true)
				{
					try
					{
						removeChild(adaptvPlayer3);
					}
					catch (err:Error)
					{
						tracer("WARN", "ADAPTV Destroy Method Not Available Yet - Error: " + err.message);
					}
					try
					{
						adaptvPlayer3.destroy();
					}
					catch (err:Error)
					{
						tracer("WARN", "ADAPTV Destroy Method Not Available Yet - Error: " + err.message);
		...
		public function SetupPlay(e:Event):void
		{
			...
			mcPlay.visible = false;
			if(videoStatus.adsEnabled == true) {
				myVid.pause();
			}
			...
			{
					//&& videoStatus.preRoll=="TRUE"
					if(videoStatus.adsEnabled && videoStatus.preRollBreak != "BREAK_START") {
						tracer("INFO", "******************* NO AUTO PLAY HERE!");
						if(hasVideo == "FALSE" || videoStatus.skipAdLoadFlag == true) {
							 myVid.play();
							 mcPlay.visible = false;
						  	 mc_pp.gotoAndStop(3);
						} else {
						...
						/*
					   if(videoStatus.adsEnabled && videoStatus.playBackReady) {
			...
			else
			{
				...
				try
				{
					...
					if(videoStatus.adsEnabled && videoStatus.preRollBreak != "BREAK_START") {
					tracer("INFO", "******************* YES AUTO PLAY HERE!");
						myVid.play();
						mc_pp.gotoAndStop(3);
		...
		private function GetVarInfo():void
		{
			if (!varsLoaded || (varsLoaded && widgetMode == "TRUE"))
			{
				...
				var getVarService:String = srvString + "/svc/clip/getVars?fid=" + guid + "&pid=" + pid + debugService1;
			...
			else
			{
				...
				if(videoStatus.skipAdLoad == true) {
					adaptvPlayer3.fetchScript();
		...
		private function ParseVarInfo(e:Event)
		{
			...
			//tracer ("INFO","VARS/CLIP/ADAPTV CONFIG: "+clpXML);
			...
			thumbLdr.width=57;
			thumbLdr.height=57;
			//meta_mc.station_logo.thumbLdr.width = 57;
			//meta_mc.station_logo.thumbLdr.height = 57;
			...
			//embedUrl="http://localhost:1765/ClipPlayer/redlasso_player.swf"
			embedUrl=clpXML.vars.@embedUrl;
			trace("MAULIK embedUrl" + embedUrl);
			userId=clpXML.vars.@userId;
			trace("MAULIK userId" + userId);
			...
			// Unless Ads are true, we don't bother to setup adapTV objects as no ads will be displayed
			if (clpXML.adaptv.@ads != "FALSE")
			{
				videoStatus.adsEnabled=true;
				configureAdapTVMeta(clpXML, clipDesc, clipName, clpETime, clpSTime, m_appName, app_source, vidname, _fileId);
			} else {
				videoStatus.adsEnabled=false;
			}
			...
			else
			{
				...
				else
				{
					...
					if ((userClicked && videoStatus.skipAdLoad != true ) || (widgetMode == "TRUE" && userClicked && videoStatus.skipAdLoad != true))
					{
						fireNSLoad(m_appName);
						//GetNSConnection(m_appName);
					}
		...
		private function fireNSLoad(fileName:String):void {
			if(videoStatus.adsEnabled == false) {
				 tracer("INFO", "fireNSLOAD NO ADS - LOAD VIDEO");
				GetNSConnection(fileName);
			} else {
				if (videoStatus.APILoaded == true && videoStatus.ScriptFetched) {
					tracer("INFO", "fireNSLOAD AD API LOADED - LOAD VIDEO");
					GetNSConnection(fileName);
				} else {
					// tracer("INFO", "fireNSLOAD AD API NOT LOADED - Wait to LOAD VIDEO");
					setTimeout(fireNSLoad, 250, fileName);
				}
			}
		}
		...
		/**
		 * Handles configuring, loading of AdapTV 2.1 API
		 *
		 *@param		clpXML		XML		XML data from getVars service
		 *@param		clipDesc		String		Clip description
		 *@param		clipName		String		Clip name
		 *@param		clpETime		Number		Clip ending time
		 *@param		clpSTime		Number		Clip starting time
		 *@param		m_appName		String		FMS server application name
		 *@param		app_source		String		FMS server stream source
		 *@param		vidname			String		Clip filename
		 *@param		_fileId			String		GUID/Clip ID
		 */
		private function configureAdapTVMeta(clpXML, clipDesc, clipName, clpETime, clpSTime, m_appName, app_source, vidname, _fileId):void
		{
			tracer("DEBUG", "Configuring AdapTV Variables.");
			/* START ADAPTV */
			adaptvPlayer3=new AdaptvPlayer3(this);

			// A valid HTTP, RMTP or RTMPTE URL for the video (usually .flv) file.
			adaptvConfig.url="rtmpe://" + app_source + "/" + m_appName + "/" + vidname;
			// The unique identifier of the video (limit 64 chars)
			adaptvConfig.preload=true;
			adaptvConfig.autostartads=false;
			adaptvConfig.id=_fileId;
			// description of video
			adaptvConfig.description=clipDesc;
			// The title of the video
			adaptvConfig.title=clipName;
			// A comma-delimited list of tags/keywords
			adaptvConfig.keywords=String(clpXML.clipinfo.@keywords);
			// A comma-delimited list of top level categories    
			adaptvConfig.categories=String(clpXML.clipinfo.@categoryName);
			// The duration of video in _seconds_, if applicable
			adaptvConfig.duration=Number(clpETime - clpSTime);

			if (widgetMode != "TRUE")
			{
				// width of video frame
				adaptvConfig.width=Number(stage.stageWidth);
				// height of video frame
				adaptvConfig.height=Number(stage.stageHeight - botbar_mc.height);
			}
			else
			{
				// In Flex/AS3 container leverage the parent width/height for sizing, etc.
				adaptvConfig.width=this.parent.width;
				adaptvConfig.height=this.parent.height - botbar_mc.height;
			}

			// x/y offset of video frame - unused in our player for current implementation
			adaptvConfig.x=0;
			adaptvConfig.y=0;

			adaptvConfig.videoPlayerId=String(clpXML.adaptv.@videoPlayerId);
			adaptvConfig.key=String(clpXML.adaptv.@key);
			adaptvConfig.zid=String(clpXML.adaptv.@zid);
			adaptvConfig.adaptag=String(clpXML.adaptv.@adaptag);
			// Handler parsing of context from adaptv node in getVars service results
			var context:Object = new Object();
			var tempItem:Array;
			var tempContextString = String(clpXML.adaptv.@context).replace("{", "");
			tempContextString=tempContextString.replace("}", "");

			var tempContext:Array = tempContextString.split(",");
			var loop:Number=0;

			for (loop=0; loop < tempContext.length; loop++)
			{
				tempItem=tempContext[loop].split(":");
				context[tempItem[0]]=tempItem[1];
			}
			adaptvConfig.context=context;

			if (String(clpXML.adaptv.@companionID) == "FALSE")
			{
				adaptvConfig.companionId="";
			}
			else
			{
				adaptvConfig.companionId=String(clpXML.adaptv.@companionID);
			}
			adaptvConfig.pageUrl=embedPage;

			// OLD hardcoded values for testing/reference
			// adaptvConfig.videoPlayerId = "Redlasso";
			// adaptvConfig.key = "poop_integration_test";
			// adaptvConfig.zid = "test.flash_ads";
			// adaptvConfig.adaptag = "partner1, tag1";
			// adaptvConfig.companionId = "altAdapTVAds";
			// var adaptvContext:Object = {key1:"value1",key2:"value2"};
			// adaptvConfig.context = adaptvContext;
			// adaptvConfig.pageUrl = "http://clients.impossibilities.com/redlasso/2009/adaptv21.php";


			videoStatus.preRoll=String(clpXML.adaptv.@preRoll).toUpperCase();

			//videoStatus.preRoll = "FALSE";
			...
			if (clpXML.adaptv.@ads != "FALSE")
			{
				...
				if (widgetMode != "TRUE")
				{
					adaptvPlayer3.setContentSize(stage.stageWidth, stage.stageHeight - botbar_mc.height, false);
				}
				else
				{
					adaptvPlayer3.setContentSize(this.parent.width, this.parent.height - botbar_mc.height, false);
				}

				this.addChild(adaptvPlayer3);
				videoStatus.ScriptFetched = false;
			...
			//videoStatus.adsEnabled=false;
			tracer("DEBUG", "AdapTV Configuration complete.");
		}
		...
		/**
		 * Handler for when statinlogo has been loaded
		 *
		...
		private function UpdateCurrentTime(e:Event)
		{
			...
			if (e.target.playheadTime > globalStop || e.target.playheadTime >= myVid.totalTime)
			{
				...
				if (videoStatus.adsEnabled)
				{
					if (e.target.playheadTime >= globalStop)
					{
						videoFinished();
						adaptvPlayer3.contentEnded();
					}
					videoStatus.startAds=true;
						//adaptvPlayer3.fetchScript();

					/*
					   if (videoStatus.firstPass) {
					   adaptvPlayer3.fetchScript();
					   } else {
					   setTimeout(deferredAdFetch, 2000);
					   }
					 */
				}


					//adaptvPlayer3.startBreak ();
			}
		/* END ADAPTV */
		}


		function deferredAdFetch():void
		{
			adaptvPlayer3.fetchScript();
		}
		...
		/* START ADAPTV */
		private function videoFinished()
		{
			if (videoStatus.adsEnabled)
			{
				tracer("INFO", "videoFinished");

				if (videoStatus_done == true)
				{
					return;
				}
				adaptvPlayer3.contentEnded();
		...
		private function logInitialPlay(evt:Event):void
		{
			...
			if (!previewPersistent)
			{
				if (myVid.alpha != 1 && videoStatus.preRollBreak != "BREAK_START")
				{
		...
		private function playClip(e:Event)
		{
			...
			/* START ADAPTV */
			if (videoStatus.startAds == true)
			{
				disableControls();
				adaptvPlayer3.startContentPlayheadChanged(this.getVideoStatus);
			}
			else
			{
				try
		...
		public function ToggleFullScreen(e:Event)
		{
			...
			if (widgetMode != "TRUE")
			{
				...
				if (videoStatus.adsEnabled)
				{
					adaptvPlayer3.setContentSize(sw, sh - botbar_mc.height, bigScreen);
				}
		...
		private function showMenu():void
		{
			...
			if (!userClicked)
			{
				videoStatus.skipAdLoad = true;
				videoStatus.skipAdLoadFlag = true;
				GetVarInfo();
			} else {
				videoStatus.skipAdLoad = false;
			}
			fadeOverlay(false);
		...
		private function email_send(e:Event)
		{
			...
			{
				var Emailxml:XML =<root rid="" sid="00000000-0000-0000-0000-000000000000"><message addresses=" " emailFrom=" "></message></root>;
				Emailxml.@rid=guid;
		...
		private function assignEmbed()
		{
			...
			trace("Maulik OBJECTTAG" + embedVideo.embedTxt.text);
			
			embedVideo.WPembedTxt.text="[redlasso id=\"" + _fileId + "\"" + "]";
			
			trace("Maulik VEDIO" + embedVideo.WPembedTxt.text);
		}
		...
		/* Required if HTML integrated:
		   adaptvPlayer3.setHTMLSize(playerFrame.width, playerFrame.height, playerFrame.x, playerFrame.y);
		   adaptvPlayer3.x = playerFrame.x;
		   adaptvPlayer3.y = playerFrame.y;
		 */
		...
		private function onLoadAudioPreviewComplete(event:Event):void
		{
			...
			//setChildIndex(overlay_mc, getChildIndex(prevLoader)+1);
		...
		/* START ADAPTV */
		//Ad script fetched:
		// Some items not needed until we re-architecht initial video loading to defer until 
		// AdapTV API has loaded
		// in order to properly support pre-rolls without gaps/or initial playback of 
		// video at beginning prior to pre-roll ad
		
		public function adapTVLoaded():void {
				videoStatus.ScriptFetched = false;
				videoStatus.APILoaded = true;
			
		}

		public function onScriptFetched(evt:Object)
		{
			videoStatus.ScriptFetched = true;
			videoStatus.APILoaded = true;
			tracer("INFO", "Ads onScriptFetched:\n" + evt);

			// To inform that an ad break is available call:
			if (videoStatus.firstPass == true && videoStatus.playBackReady == false)
			{
				tracer("INFO", "STARTING PRE-ROLL BREAK");
				videoStatus.preRollBreak = "BREAK_NULL";
				adaptvPlayer3.startBreak();
			}
			else
			{
				tracer("INFO", "NO PRE-ROLL BREAK");
				adaptvPlayer3.contentEnded();
				adaptvPlayer3.endBreak();
				videoStatus.playBackReady=true;
			}
		...
		//Linear ad break started: Again not needed for overlays only pre-roll
		public function onBreakStarted(evt:Object)
		{
			tracer("INFO", "onBreakStarted:\n" + evt);
			videoStatus.preRollBreak = "BREAK_START";
			videoStatus.playBackReady=true;
		...
		The way to determine if a pre-roll exists is call adPlayer.startBreak();
		
		If there's a pre-roll then the event onBreakStarted is called followed by
		the onBreakEnded event.
		
		If there's no pre-roll then only the onBreakEnded event is called.
		
		This also works for determining other linear events like post-roll.

		*/

		//Linear ad break ended: Again not needed for overlays only pre-roll
		public function onBreakEnded(evt:Object)
		{
			tracer("INFO", "OnBreakEnded:\n" + evt);
			
			
			// The player can now play and controls should be enabled
			adaptvPlayer3.endBreak();
			videoStatus.startAds=false;
			enableControls();
			mc_pp.gotoAndStop(2);
			//myVid.play();

			if (videoStatus.adsEnabled && videoStatus.playBackReady ) //&& videoStatus.preRoll == "TRUE"
			{
				tracer("INFO", "OnBreakEnded PLAY START");
				mcPlay.visible=false;
				mc_pp.gotoAndStop(3);
				myVid.play();
				...
				tracer("INFO", "THIS IS WHERE!")
			}
			if(videoStatus.adsEnabled==true && hasVideo=="FALSE") {
				tracer("INFO", "THIS IS WHERE #2: "+myVid.state)
					// userClicked = true;
					mcPlay.visible=false;
					myVid.pause();
					myVid.play();
			...
			if(videoStatus.preRollBreak == "BREAK_NULL") {
				// jumped right here because no pre-roll
					videoStatus.preRollSKIP = true;
			}
			
			videoStatus.preRollBreak = "BREAK_ENDED";
			tracer("INFO", "OnBreakEnded contentStarted");
			// To inform that the clip is playing and available for overlay call: 
			adaptvPlayer3.contentStarted();
			tracer("INFO", "content started - vidstate: "+myVid.state)
		}


		//Custom ad plugin message:
		public function onCustom(evt:Object)
		{
			tracer("INFO", "onCustom:\n" + evt);
			// Process the custom data sent in the event object, evt
		}


		// Ad click through event:
		public function onClickThru(evt:Object)
		{
			tracer("INFO", "onClickThru");
			tracer("INFO", "click_thru=" + evt.data.url);
			this.externalPlay("PAUSE");
			// evt.data.url contains the ad click through url.
		}


		//Ad linear change event:
		public function onLinearChanged(evt:Object)
		{
			tracer("INFO", "onLinearChanged:\n" + evt);
			// evt.data.linear is whether the ad is linear or not (true/false.)
		}


		//Ad companion information event:
		public function onCompanion(evt:Object)
		{
			tracer("INFO", "OnCompanion:\n" + evt);
		...
		//No ads available:
		public function noAds(info:Object=null):void
		{
			videoStatus.ScriptFetched = false;
			tracer("INFO", "noAds: " + info+ ":: "+info.type+" :: "+info.description);
			// info.type is either "adPlayerLoadingTimeout" or "scriptFetchTimeOut".
			// info.description is a string explanation for ads not being available.
			// A variable flag can be set here to signify that ads are not available for this view for code conditions like the play pausing:
			// adAvailable = false;
			
			//Just because we have noAds, doesnt mean we shouldn't play the clip...
			tracer("INFO", "AdapTV failed to load, so disable ads and continue...");
			videoStatus.adsEnabled = false;
			videoStatus.startAds = false;
			playClip(null);
		}
		...
		public function disableControls():void
		{
			...
			botbar2_mc.visible=true;
			botbar2_mc.enabled = true;
			botbar2_mc.useHandCursor = false;
		...
		{
			if (Math.ceil(myVid.playheadTime) >= globalStop)
			{
				videoFinished();
			}
