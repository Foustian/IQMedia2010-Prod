...
{
    public class GetHighlightedTW : ICommand
    {
        ...
        public GetHighlightedTW(object ID, object Format, object TweetID, object SearchTerm)
        {
        ...
        public void Execute(HttpRequest HttpRequest, HttpResponse HttpResponse)
        {
            ...
            try
            {
                ...
                if (_ID != null)
                {
                    ...
                    {
                        string highlight = highlightedTWLogic.GetTWHighlight(iQAgentSearchTerm.SearchTerm, iQAgentSearchTerm.ArticleID, -1, out hasResult, BaseCommand.GeneratePMGUrl(BaseCommand.PMGUrlType.TW.ToString(), null, null));
                        highlightedTWOutput.Text = highlight;
                    }
                }
                else
                {
                    string highlight = highlightedTWLogic.GetTWHighlight(_SearchTerm, _TweetID, -1, out hasResult, BaseCommand.GeneratePMGUrl(BaseCommand.PMGUrlType.TW.ToString(), null, null));
                   highlightedTWOutput.Text = highlight;
                }
