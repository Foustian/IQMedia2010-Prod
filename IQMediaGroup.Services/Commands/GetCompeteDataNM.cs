...
{
    public class GetCompeteDataNM : ICommand
    {
        public String _Format { get; private set; }

        public GetCompeteDataNM(object Format)
        {
        ...
        public void Execute(HttpRequest HttpRequest, HttpResponse HttpResponse)
        {
            ...
            try
            {
               ...
               /* bool? HasAccess = _CompeteDataLogic.CheckClientCompeteDataAccess(_CompeteInput.ClientGuid.Value);
                ...
                {*/
                _CompeteOutput = _CompeteDataLogic.GetCompeteByListNM(_CompeteInput, BaseCommand.GeneratePMGUrl(BaseCommand.PMGUrlType.MO.ToString(), (!string.IsNullOrEmpty(_CompeteInput.FromDate) ? (DateTime?)Convert.ToDateTime(_CompeteInput.FromDate) : null), (!string.IsNullOrEmpty(_CompeteInput.ToDate) ? (DateTime?)Convert.ToDateTime(_CompeteInput.ToDate) : null)));
                /*}
                else
                {
