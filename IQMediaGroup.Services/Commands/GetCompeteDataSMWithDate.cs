...
{
    public class GetCompeteDataSMWithDate : ICommand
    {
        public String _Format { get; private set; }

        public GetCompeteDataSMWithDate(object Format)
        {
        ...
        public void Execute(HttpRequest HttpRequest, HttpResponse HttpResponse)
        {
            ...
            try
            {
                ...
                FromDate = !string.IsNullOrEmpty(competeInput.FromDate) ? (DateTime?)Convert.ToDateTime(competeInput.FromDate) : null;
                ToDate = !string.IsNullOrEmpty(competeInput.ToDate) ? (DateTime?)Convert.ToDateTime(competeInput.ToDate) : null;
                string _PmgUrl = BaseCommand.GeneratePMGUrl(BaseCommand.PMGUrlType.MO.ToString(), FromDate, ToDate);
                competeOutput = competeDataLogic.GetCompeteByListSM(competeInput, _PmgUrl);
                /*}
                else
                {
