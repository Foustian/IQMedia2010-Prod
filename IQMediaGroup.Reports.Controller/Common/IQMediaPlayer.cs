...
namespace IQMediaGroup.Reports.Controller.Common
{
    public class IQMediaPlayer
    {
        ...
        public static string RenderClipPlayer(Guid? ClientGuid, string p_UserID, string p_ClipID, string IsRawMedia, string PageName, string _toEmail, string ServicesBaseURL, bool? IsActivePlayerLogo, string PlayerLogoImage, bool IsMicrosite = false, bool EB = true)
        {
            try
            {
                ...
                _StringBuilder.Append("<object id=\"HUY\" name=\"HYETA\" type=\"application/x-shockwave-flash\" ");
                _StringBuilder.AppendFormat("width=\"{0}\" ", ConfigurationManager.AppSettings["ClipObjectWidth"].ToString());
                _StringBuilder.AppendFormat("height=\"{0}\" ", ConfigurationManager.AppSettings["ClipObjectHeight"].ToString());
                ...
                if (Convert.ToBoolean(ConfigurationSettings.AppSettings["IsLocal"]) == true)
                {
                    ...
                    {
                        _StringBuilder.AppendFormat("data=\"{0}\" ", ConfigurationManager.AppSettings["ResizePlayerLocation"].ToString());
                    }
                }
                else
                {
                ...
                if (Convert.ToBoolean(ConfigurationManager.AppSettings["IsLocal"]) == true)
                {
                    ...
                    {
                        _StringBuilder.AppendFormat("src=\"{0}\" ", ConfigurationManager.AppSettings["ResizePlayerLocation"].ToString());
                    }
                }
                else
                {
        ...
        public static string RenderIQAgentPlayer(string embedId, string ServicesBaseURL, string autoPlayback)
        {
            try
            {
                string _ParamValue = "embedId=" + embedId + "&ServicesBaseURL=" + ServicesBaseURL + "&autoPlayback=" + autoPlayback;
