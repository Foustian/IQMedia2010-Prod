...
void Capture::GetCaptureDevices()
{
	...
	// lets get all capture devices of system and then we loop them.
    IEnumMoniker *pEm=0;
	...
	// loop trough all the capture devices using moniker and all all monikers to list ,
	// later we use them for making our Video Capture Source Filter
    while(hr = pEm->Next(1, &pM, &cFetched), hr==S_OK)
    {
        ...
        if(SUCCEEDED(hr))
        {
            ...
            if(hr == NOERROR)
            {
				// we will add them to array list , so we can use it later
	...
	// as x264vfw is not directshow filter , we can not create them directly using GUID
	// as others we do... so we can get it using monikers 
	...
	if (hr == S_OK)  // S_FALSE means nothing in this category.
	{
		while (S_OK == pEnum->Next(1, &pMoniker, NULL))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				{
					// yuppeeee , we get x264vfw filter , we can get its filter object here only , 
...
EnumAudio:
    ...
    // the another strange this about osprey audio devices is , we do not get from audio devices
	// so we get all streaming devices (which include all audio and video devices)
	// and then add in list out audio devices only.
    ...
    while(hr = pEm->Next(1, &pM, &cFetched), hr==S_OK)
    {
        ...
        // devices with VFW drivers are NOT added
            ...
            if(hr == NOERROR)
            {
				...
				if (strstr(nameBuf,"Microphone") != NULL)
...
BOOL Capture::InitCapFilters()
{
	...
	// what we do is , intailly we'll create instace for all filters (just one  time for appliction)
...
void Capture::SetFormatSize()
{
 ...
 /* AM_MEDIA_TYPE *pmtPreview;
  ...
  if (pVSC && pVSC->GetFormat(&pmt) == S_OK)
  {
	...
	// if both specified then we'll also set that for our output file
...
BOOL Capture::GetCaptureFiltersAndSource()
{
	...
	if (pOCD)
	{
		...
		// now set framerate and height / width of output file.
		SetFormatSize();
...
BOOL Capture::MakeGraphInstances()
{
	...
	// and then just included header file 'MonoGramAACConfig.h' 
	// and then we need its interface , and can call its method for settings configuration
...
BOOL Capture::BuildMp4CaptureGraph()
{
	...
	// first we need video and audio capture sources and theri StramConfig objects
	...
	// find capture pin connect to xcodec filter
	IPin *pOut = NULL;
	...
	// find output pin of xcodec filter and connect it to mp4mux filter
	...
	// find output pin of audio filter and connect it to acm wrapper.
	IPin *pAOut = NULL;
	...
	hr = ConnectFilters(pFg,pAOut,g_MonoAACEndcoder);
	if(FAILED(hr))
	{
		LogMsg("Error : Cannot connect Audio Filter and Monogram AAC Encoder");
		return FALSE;
	...
	if(FAILED(hr))
	{
		LogMsg("Error : Cannot find unconnected output pin on Monogram AAC Encoder");
		return FALSE;		
	}
	...
	// set file names varibales for our CC file and Capture file for 
	// format will be like 
	//StationID_YearMonthDay_HourMinute.txt
	//like StationID_YearMonthDay_HourMinute.mp4
	SYSTEMTIME st;
	FILETIME ft;
	time_t localtime = 0;
	time(&localtime);
...
{    
   ...
   // turn the capture pin on now!
		...
		// each filter graph inbuilt make refernce clock that stramline its all capture 
