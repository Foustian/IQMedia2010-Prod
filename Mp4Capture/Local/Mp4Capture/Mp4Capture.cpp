...
BOOL Mp4Capture::InitInstance()
{
	...
	// get the drive for application and read app.config file on application directory location
	int drive = _getdrive();
	size_t n;
	...
	TRY
	{
		// we have read all the config params , now we will assign them to our capture object to use them there further. 
		...
		/// this we have used for preview , now not needed any window.
		//capInstance->ShowWindow(SW_SHOW);
		//capInstance->UpdateWindow();
	

		// We need to set settings for x264vfw filter programtically (we have included x264vfwConfig.h header file for all its required parameters)
