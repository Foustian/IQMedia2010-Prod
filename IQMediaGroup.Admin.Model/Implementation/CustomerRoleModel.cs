    ...
    internal class CustomerRoleModel : IQMediaGroupDataLayer, ICustomerRoleModel
    {
        ...
        public string UpdateCustomerRoleByClientIDRoleID(CustomerRoles p_CustomerRoles)
        {
            try
            {
                string _Result = string.Empty;
                List<DataType> _ListOfDataType = new List<DataType>();
                _ListOfDataType.Add(new DataType("@CustomerID", DbType.Int32, p_CustomerRoles.CustomerID, ParameterDirection.Input));
                _ListOfDataType.Add(new DataType("@RoleID", DbType.Int32, p_CustomerRoles.RoleID, ParameterDirection.Input));

                _Result = ExecuteNonQuery("usp_CustomerRole_UpdateByClietIDRoleID", _ListOfDataType);
