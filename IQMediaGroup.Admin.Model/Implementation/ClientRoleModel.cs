...
namespace IQMediaGroup.Admin.Model.Implementation
{
    ...
    internal class ClientRoleModel : IQMediaGroupDataLayer, IClientRoleModel
    {
        ...
        public string UpdateClientRoleByClientIDRoleID(ClientRoles p_ClientRole)
        {
            try
            {
                string _Result = string.Empty;
                List<DataType> _ListOfDataType = new List<DataType>();
                _ListOfDataType.Add(new DataType("@ClientID", DbType.Int32, p_ClientRole.ClientID, ParameterDirection.Input));
                _ListOfDataType.Add(new DataType("@RoleID", DbType.Int32, p_ClientRole.RoleID, ParameterDirection.Input));

                _Result = ExecuteNonQuery("usp_ClientRole_UpdateByClietIDRoleID", _ListOfDataType);
        ...
        public DataSet GetClientRoleByClientID(Int64 p_ClientID)
        {
            try
            {
                DataSet _DataSet = new DataSet();
                ...
                _DataSet = this.GetDataSet("usp_ClientRole_SelectRoleByClientID", _ListOfDataType);
